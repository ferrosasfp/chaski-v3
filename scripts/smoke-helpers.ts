 ...
 * (`REAL_PAYOUT_PROVENANCES`, src/presentation/flow-vm.ts:22), donde lo desconocido cae del lado que
 ...
 *
 * Comparación EXACTA, la misma que `Set.has` en flow-vm.ts:27. No se normaliza a minúsculas a
...
export function classifyPayoutProvenance(
  ...
  if (REAL_PAYOUT_PROVENANCES.has(provenance)) {
    return {
      ...
      reason:
        ...
        "(src/presentation/flow-vm.ts:22), el conjunto que producción trata como desembolso fiat " +
  ...
  return {
    ...
    reason:
      ...
      `(src/presentation/flow-vm.ts:22) ni entre las no-reales conocidas (${known}). La comparación ` +
