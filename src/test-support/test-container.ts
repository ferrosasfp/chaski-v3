...
import type {
  ...
  SolanaCloseableEscrowLister, SolanaEscrowChainStateReader, ConnectedWalletProbe, SolanaRentReader, // HU-079: EN ESTA LÍNEA por lo mismo. // WKH-349: EN ESTA LÍNEA, no en una nueva — mismo motivo que el import de `:7`: `container.test.ts:413` cita `test-container.ts:87` por número. WKH-354: `ConnectedWalletProbe` entra ACÁ por lo mismo
...
export interface TestContainerOverrides {
  ...
  solanaCloseableEscrows?: SolanaCloseableEscrowLister; solanaEscrowStates?: SolanaEscrowChainStateReader; connectedWallet?: ConnectedWalletProbe; recorridoPorEnlace?: PreparacionPorEnlace; solanaRent?: SolanaRentReader; // HU-079: EN ESTA LÍNEA por lo mismo. Es OPCIONAL sólo acá —en `../composition/container.ts:79` va SIN `?`—, y su default NO es `undefined`: es `new FakeSolanaRentReader()`, que contesta lo que la cadena contestó el 2026-09-04. Un default `undefined` haría que todo test que no lo declare ejercite el camino degradado sin saberlo, y el camino degradado es justo el que NO muestra cifra. // WKH-358: `recorridoPorEnlace` entra ACÁ por lo mismo. WKH-349: EN ESTA LÍNEA (`:87` se cita por número). Misma disciplina que sus dos vecinos: sin override queda UNDEFINED ⇒ el historial NO pregunta nada a la cadena y dice el copy de siempre. WKH-354: `connectedWallet` entra ACÁ por lo mismo; su default NO es undefined sino `new FakeConnectedWallet(null)` = "no hay ninguna billetera conectada", que es el estado real de un test que no montó ningún árbol
...
export function buildTestContainer(o: TestContainerOverrides = {}): Container {
  ...
  const base: Container = {
    ...
    connectedWallet: o.connectedWallet ?? new FakeConnectedWallet(null), recorridoPorEnlace: o.recorridoPorEnlace ?? new RecorridoPorEnlaceNulo(), solanaRent: o.solanaRent ?? new FakeSolanaRentReader(), // HU-079 — EN ESTA LÍNEA, misma disciplina que sus vecinos. // WKH-358 — EN ESTA LÍNEA, misma disciplina que sus vecinos. El default NO es `undefined` ni el objeto real: es el doble que dice "esta persona no eligió ninguna billetera por enlace", que es el estado real de un test que no montó ningún selector
  };
