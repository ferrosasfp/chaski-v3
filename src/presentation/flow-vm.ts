 ...
 *    `maxCandidates` PDAs on-chain (`resolveRemittanceIdFromLedger`, `solana-wallet.ts:323`). `escrow_not_found` sale de DOS
...
export function lostEscrowRecoveryError(code: string, maxCandidates: number): string {
  ...
  // (`connect`, `solana-wallet.ts:170`): `wallet_bridge_not_mounted` lo tira `openModal` si el árbol
 ...
 * dice — es la misma trampa que `probeEscrowRefunded` ya tiene escrita (`probeEscrowRefunded`, `solana-wallet.ts:806`).
 */
