...
describe("POST /api/payout/validate — autoridad server-side (WKH-180)", () => {
  ...
  // ── Guard 2: formato ────────────────────────────────────────────────────────
  it("verificationId '' (con key) → 400 invalid_verification_id, fetch NOT called (AC-5)", async () => {
    vi.stubEnv("DIDIT_API_KEY", "test-key");
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const res = await POST(req({ verificationId: "", address: ADDR }));
    expect(res.status).toBe(400);
    expect(await res.json()).toEqual({ authorized: false, reason: "invalid_verification_id" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("verificationId ausente (con key) → 400 invalid_verification_id, fetch NOT called (AC-5)", async () => {
    vi.stubEnv("DIDIT_API_KEY", "test-key");
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("key + Didit Declined → 200 authorized:false kyc_not_approved (AC-2, AC-6)", async () => {
    vi.stubEnv("DIDIT_API_KEY", "test-key");
    vi.stubGlobal("fetch", diditOk({ status: "Declined", session_id: VID }));
    const res = await POST(req({ verificationId: VID, address: ADDR }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ authorized: false, reason: "kyc_not_approved" });
  });

  it("key + Didit !res.ok → 502 authorized:false kyc_reauth_failed (AC-2)", async () => {
    vi.stubEnv("DIDIT_API_KEY", "test-key");
  ...
  it("vendor_data mismatch vs address → false kyc_ownership_mismatch (AC-1 ownership)", async () => {
    vi.stubEnv("DIDIT_API_KEY", "test-key");
    vi.stubGlobal("fetch", diditOk({ status: "Approved", session_id: VID, vendor_data: "0xOtherWallet" }));
    const res = await POST(req({ verificationId: VID, address: ADDR }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ authorized: false, reason: "kyc_ownership_mismatch" });
  });
