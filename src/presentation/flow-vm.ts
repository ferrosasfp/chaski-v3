...
export function humanError(code: string): string {
  ...
  // `payout_failed` cuyo reason no reconozca (`humanError`, `flow.tsx:1651`), justo cuando no sabemos dónde está la
 ...
 *  USDC.", `flow.tsx:2542`), aunque nombre la misma acción. Mi primera versión la copiaba tal cual para
 ...
 * que no lleva a ningún refund: `settled` no está en (`refundeable`, `flow.tsx:1455`), que es
 ...
 * `flow.tsx:873`), en la pantalla de inicio. No mira el estado local de la remesa: resuelve el id
 ...
 * (`idsAConsultar`, `flow.tsx:3127`), y (`escrowOutcome`, `:1193`) devuelve el conocimiento local tal cual con `"not-asked"`)
