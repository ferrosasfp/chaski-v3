...
//   · "Ver mis envíos"                                    (`flow.tsx:857`)
//   · "Recuperar un envío perdido"                        (`flow.tsx:2283`)
//   · "Recuperar el depósito de red de envíos anteriores"  (`flow.tsx:2433`)
// Y otros tres viven apretados en una fila del header, uno de ellos destructivo:
//   · "Borrar igual" (borra los datos locales)             (`flow.tsx:675`)
//   · "Cancelar"                                           (`flow.tsx:682`)
//   · "¿No sos vos?"                                       (`flow.tsx:692`)
//
...
describe("T-341-3 (AC-2): las tres puertas de recuperar plata declaran alto de toque", () => {
  it("las tres declaran min-h de 52 px o más", () => {
    ...
    // (`className`, `flow.tsx:857`), (`className`, `flow.tsx:2283`), (`className`, `flow.tsx:2433`)
 ...
 * (`confirmReset`, `flow.tsx:667`), y la otra rama es la que pinta "¿No sos vos?"
 * (`onAskReset`, `flow.tsx:691`)). Así que el px de "¿No sos vos?" se lee ANTES del click y
...
async function pxDelBloqueDeReset(): Promise<{
  ...
  // lo que la resuelve: conecta la wallet y hace `setAddress` (`resolveSender`, `flow.tsx:387`).
...
describe("T-341-5 (AC-3): los tres controles del bloque de reset llegan al mínimo de toque", () => {
  it("los tres declaran min-h de 44 px o más", async () => {
    ...
    // header (`flow.tsx:670`) y a 52 cada uno el header crece más de lo necesario.
    //
    // INPUT QUE LO PONE EN ROJO: dejar cualquiera de los tres sin `min-h-`
    // (`className`, `flow.tsx:675`), (`className`, `flow.tsx:682`), (`className`, `flow.tsx:692`)
