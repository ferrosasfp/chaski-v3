...
export function humanError(code: string): string {
  ...
  // `payout_failed` cuyo reason no reconozca (`humanError`, `flow.tsx:1608`), justo cuando no sabemos dónde está la
 ...
 *  USDC.", `flow.tsx:2499`), aunque nombre la misma acción. Mi primera versión la copiaba tal cual para
 ...
 * que no lleva a ningún refund: `settled` no está en (`refundeable`, `flow.tsx:1412`), que es
 ...
 * `flow.tsx:830`), en la pantalla de inicio. No mira el estado local de la remesa: resuelve el id
 ...
 * (`idsAConsultar`, `flow.tsx:3084`), y (`escrowOutcome`, `:1193`) devuelve el conocimiento local tal cual con `"not-asked"`)
