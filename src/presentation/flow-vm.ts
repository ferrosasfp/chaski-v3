import type { Money } from "../domain/money"; import type { EscrowChainState } from "../application/ports"; import type { CausaDeEnlaceEnPantalla } from "../infrastructure/solana/deeplink/firma-por-enlace"; // WKH-358/AC-8 agregó el último EN ESTA LÍNEA y ANTES de este comentario, por lo mismo. WKH-349: EN ESTA LÍNEA, no en una nueva — `:25`, `:29`, `:30`, `:206`, `:253`, `:327`, `:329` y `:1144` de este archivo los cita otro por número y están TODOS debajo de acá
 ...
 * dice — es la misma trampa que `probeEscrowRefunded` ya tiene escrita (`probeEscrowRefunded`, `solana-wallet.ts:1493`).
 */
...
export function humanError(code: string): string { const porEnlace = copyDeEnlace(code); if (porEnlace !== null) return porEnlace; // WKH-358/AC-8/DT-8 — EN ESTA LÍNEA (Δ0: el primer `if` de abajo es `:573` y una línea acá rompe 24 citas ancladas) y PRIMERO, antes de la cadena de `includes`. Es un lookup EXACTO, así que no puede robarle un código a nadie; lo que sí puede pasar al revés es que un needle nuevo de abajo sea subcadena de una de las causas del `Record` y se las robe en silencio. Medido: hoy ninguna de ellas contiene ninguno de los needles actuales, o sea que HOY no hay colisión — pero el orden de una cadena de `includes` es una propiedad frágil y esto la fija. Lo mide `T-065-COPY-4`
  ...
  // carril se borró.
  //
  // ⚠️ WKH-335 NO LOS RESUCITA, y conviene tenerlo claro para no reintroducirlos: lo que `/compose`
  // manda ahora es `agentFailure`, una CLASE de dos valores (`INPUT_REJECTED` / `AGENT_ERROR`), NO el
  // `reason` del agente. El vocabulario `fx_*` SIGUE sin llegar, así que sigue sin haber productor de
  // esos dos códigos y el copy de abajo sigue siendo el más preciso que el dato sostiene: "el
  // corredor lo rechazó, probá con otro monto" — sabemos que rechazó el CONTENIDO del pedido, no
  // sabemos qué campo.
  //
  // ✅ Lo que WKH-335 SÍ cambió: `a2a_quote_rejected` volvió a tener productor vivo
  // (`app/api/a2a/quote/route.ts`), así que este `if` dejó de ser sólo un lector del historial local.
  //
  ...
  // ahora lee el default genérico en vez del copy del mínimo. Sigue siendo cierto después de
  // WKH-335, y se acepta por la misma razón: el alternativo es una frase que afirma más que el
  ...
  // el escrow.
  // El número NO está escrito a mano: se formatea desde `SENDER_MIN_LAMPORTS_FOR_DEPOSIT`, que desde
  // HU-079 ya NO es el umbral que compara el guard sino el RESPALDO. Que se derive lo mide `T-347-14`.
  ...
  if (code.includes("solana_sender_sol_insufficient"))
    return `Te falta SOL en la wallet: de referencia hacen falta unos ${formatLamportsAsSol(SENDER_MIN_LAMPORTS_FOR_DEPOSIT)} SOL, y puede que la red pida más o puede que pida menos: cuánto exactamente lo decide ella en el momento. La comisión de red del depósito la pagamos nosotros; de tu wallet salen el alquiler de las cuentas del escrow y la comisión de la transacción con la que podrías recuperar tus USDC. Cargá SOL con un poco de margen y volvé a intentar: no se movió ningún USDC.`; // 🔴 HU-079, EN ESTA MISMA LÍNEA (Δ0). ⛔ EL CUANTIFICADOR SE CORRIGIÓ DOS VECES Y ÉSTA ES LA SEGUNDA (fix-pack AR/BLQ-BAJO-1). Decía «necesitás AL MENOS X», que es falso hacia ABAJO (hoy devnet exige 6.503.880, o sea MENOS que este número), y la HU-079 lo pasó a «con X te alcanza seguro», que es falso hacia ARRIBA y peor: justificaba la promesa afirmando que el respaldo es una COTA SUPERIOR, y el docblock de (`senderMinLamportsForDeposit`, `../application/solana-escrow-rent.ts:241`) dice lo contrario con todas las letras («6960 fue el máximo histórico, no una cota»). Medido con el factor 7500 que ese mismo docblock nombra como posible: el guard exige 9.555.500 y el respaldo son 8.874.560 ⇒ la persona carga lo que el mensaje le promete como suficiente y vuelve a chocar contra el mismo guard, sin ninguna pista de cuánto falta. ⛔ Esta función recibe un `code: string` y nada más, así que no se le puede pasar la lectura de cadena sin tocar sus ~30 call-sites: imprime el RESPALDO, lo dice COMO valor de referencia y nombra las DOS direcciones, que es lo único verdadero pase lo que pase con la tarifa. ⛔ NINGUNA afirmación de suficiencia sin testigo: lo mide `T-079-C1` al final de `flow-vm.test.ts`, cuyo `it` (a) se pondría rojo el día que el respaldo SÍ fuera una cota.
  ...
  if (code.includes("payout_not_authorized"))
    return "Tu verificación de identidad no autoriza este envío. No se movió ningún USDC de tu wallet."; if (code.includes("payout_pop_unavailable") || code.includes("prepare_unavailable")) return "No llegamos a pedir la orden de pago, así que no se movió ningún USDC de tu wallet y no hay ningún reembolso pendiente. Puede ser que la red no nos contestara, que este navegador no pueda dar la prueba de billetera que el envío necesita, o que el problema sea nuestro y esa prueba esté sin configurar de nuestro lado. Si volviste de tu app de billetera, ese camino todavía no completa el pago: abrí Chaski adentro de Phantom y probá desde ahí."; // WKH-358 (fix-pack · AR/BLQ-MED-2) — EN ESTA LÍNEA, Δ0: una línea nueva acá corre las citas por número de este archivo que apuntan más abajo. ⚠️ ACÁ DECÍA «15 citas (`:953` a `:1377`)» Y NO SE RE-DERIVA DE NINGUNA LECTURA (re-AR it2 · BLQ-BAJO-4). RE-MEDIDO en el árbol de este commit con el instrumento de `citas-ancladas.test.ts` (su regex `ANCLADA` + su resolución de destino; entrantes largas + auto-citas ancladas, filtradas por destino): las que apuntan más abajo de esta línea son **24 ocurrencias a 10 destinos, de `:1145` a `:1377`**; el archivo entero recibe 51 (27 largas de 15 archivos + 24 auto) a 23 destinos. El invariante que justifica el Δ0 no es el total: es que todo lo que está más abajo se corre con una línea nueva acá. ⛔ VA ANTES del catch-all `code.includes("payout")` de abajo, que es el que se tragaba `payout_pop_unavailable` y le daba el copy de un payout FALLADO ("si tus USDC entraron al escrow…"): los dos enums de acá traen `principal: "not_deposited"` por construcción, así que esa frase mandaba a buscar plata a un lugar donde con CERTEZA no hay plata. ⚠️ Y NO dice "no se pidió ninguna firma", que sería lo simétrico con `prepareRejected` y sería FALSO. ⛔ PERO EL FUNDAMENTO NO ES «`prove()` SÍ pide firmar», que es demasiado ancho (re-AR it2 · MNR-4): (`prove`, `../infrastructure/auth/http-pop-signer.ts:16`) tiene TRES salidas y sólo UNA toca la billetera. Las otras dos cortan antes de (`signMessage`, `../infrastructure/auth/http-pop-signer.ts:29`): el 501 de `:22` (nuestro server sin `PAYOUT_POP_SECRET`) y el `!ok`/red caída de `:23`. Lo que hace falso negar la firma es que la tercera salida EXISTE y es alcanzable, no que las tres firmen. ⚠️ Y POR ESO EL COPY SUMA LA TERCERA CAUSA: el sub-caso 501 no es «la red no contestó» ni «este navegador no puede», es NUESTRO, y sin nombrarlo la persona se queda sin la causa real y sin señal de que el problema no es suyo. Lo que sí es certeza es que ningún USDC se movió, y eso es lo que la frase afirma
  if (code.includes("payout_authority_unavailable")) return "No llegamos a una respuesta de quien autoriza los pagos, así que el envío se cortó ahí. No se movió ningún USDC de tu wallet y no hay nada que reclamar: la firma que te pedimos antes sirvió sólo para probar que la billetera es tuya. Probá de nuevo en un rato; si te sigue pasando, vas a tener que verificar tu identidad otra vez desde esta billetera."; if (code.includes("payout")) // WKH-233 (fix-pack · H-1) — EN ESTA LÍNEA, Δ0: el primer statement de acá es el copy propio de `payout_authority_unavailable` y NO va en una línea nueva, porque una línea nueva en este punto corre las 24 citas ancladas por número que apuntan más abajo de este archivo (`:1145` a `:1377`; el instrumento es el mismo `citas-ancladas.test.ts`, y el invariante que manda no es el total sino que todo lo de abajo se desplaza). ⛔ VA ANTES del catch-all `code.includes("payout")` que lo sigue en esta misma línea: `payout_authority_unavailable` contiene "payout" y NO contiene "kyc" ni "prepare_unavailable", así que sin este `if` caía ahí y la persona leía "si tus USDC entraron al escrow, los sacás vos firmando" — mandando a buscar plata a un lugar donde con CERTEZA no hay plata: los TRES emisores del enum ((`payout_authority_unavailable`, `../../app/api/payout/prepare/route.ts:394`), (`payout_authority_unavailable`, `../../app/api/payout/prepare/route.ts:405`) y (`payout_authority_unavailable`, `../../app/api/payout/prepare/route.ts:408`); iban SIN ancla y decían `:334`/`:345`/`:348`, tres comentarios: la ola W3 de WKH-372 insertó 60 líneas más arriba y las corrió a las tres, y sin ancla el candado no las miraba — AR/BLQ-MED-1) cortan ANTES del forward al agente y ANTES de `authorizePrincipal`, o sea antes de que se firme una sola transacción. ⚠️ Y EL COPY **NO** DICE "no se pidió ninguna firma", que sería FALSO: para llegar a este guard el PoP ya se verificó ((`POP_SECRET`, `../../app/api/payout/prepare/route.ts:214`) en adelante), o sea que la billetera SÍ firmó un mensaje. Lo que es certeza es que ese mensaje no mueve valor, y por eso la frase lo nombra en vez de callarlo. ⚠️ LA SEGUNDA ACCIÓN ("verificar tu identidad otra vez") NO es relleno: el caso CD-21 ((`decisionToken`, `../infrastructure/payout/authority.ts:36`), invalidado por una rotación del secreto del agente; `:35` es una línea MUDA y la cita nació así en esta rama — re-AR it3 · MNR-1) sale por acá y NO se arregla reintentando — se arregla re-verificándose. Va SEGUNDA porque los otros dos emisores sí son transitorios y para ellos re-verificarse es un consejo caro. Lo mide `T-COPY-5`
 ...
 * (`:735`, `:859`, `:919`, `:956`) y este comentario afirmaba que "ninguno de los cuatro seguía siendo
 ...
 * `EscrowOutcome` que ya produce (`escrowOutcome`, `:1201`) y se dice a qué sección va esa fila.
 *
 * LA PARTICIÓN ES TOTAL. Los 12 valores de (`EscrowOutcome`, `:1175`) se reparten entre los 4 grupos
 ...
 * (`escrowOutcomeDisplay`, `:1260`), y eso NO es "entregado": esa palabra es de
 ...
 * hay que contarlos en (`GRUPO_POR_DESENLACE`, `:1363`) y no en este párrafo, no llegaron por el mismo motivo. `chain-absent`
 ...
 * (`idsAConsultar`, `flow.tsx:3322`), y (`escrowOutcome`, `:1201`) devuelve el conocimiento local tal cual con `"not-asked"`)
 ...
 * calla el plazo a propósito (`escrowOutcomeDisplay`, `:1260`). Afirmarlo en el encabezado
 ...
 * desenlace sólo se alcanza por filas `unverified`, porque (`escrowOutcome`, `:1201`) corta en su
 ...
 *
 * Esto NO es un bug del corte —el corte está argumentado en (`escrowOutcome`, `:1201`) y su candado es
 ...
 * (`EscrowOutcome`, `:1175`) gana un valor nuevo y nadie le da grupo, esto NO COMPILA. Un `if`-chain
...
export function historyGroupFor(o: EscrowOutcome): HistoryGroup {
  ...
  // el desenlace en (`escrowOutcome`, `:1201`): el lado seguro, el que no afirma nada sobre fondos.
...
const COPY_DE_ENLACE: Record<CausaDeEnlaceEnPantalla, string> = {
  ...
  // ── WKH-359 · LAS TRES DE LA PRUEBA DE POSESIÓN POR ENLACE ─────────────────────────────────────
  // 🔴 FALTA EL INSUMO, y por eso NO dice "cancelaste": a la persona no se le preguntó nada todavía.
  // Es PRE-SALTO por construcción (lo emite (`signMessage`, `../infrastructure/solana-wallet.ts:1922`)
  // antes de navegar a ninguna billetera), así que afirmar cualquier cosa sobre una firma sería falso.
  deeplink_pop_sin_firma:
    "Nos falta un permiso tuyo para seguir y todavía no te lo pedimos. Volvé a intentarlo: te vamos a llevar a tu billetera para que lo confirmes.",
  // 🔴 ⛔ ESTE COPY NO AFIRMA NI QUE SE FIRMÓ NI QUE NO SE FIRMÓ, y es a propósito ([NC-2] de WKH-359).
  // La causa se alcanza tanto ANTES del salto (el ancla venció esperando) como DESPUÉS (la persona
  // volvió tarde), y desde acá las dos son indistinguibles: el ancla no registra si la billetera llegó
  // a mostrar la pantalla. El vecino `deeplink_viaje_vencido` (arriba) SÍ dice "No se firmó nada", y
  // WKH-358 midió que esa frase sale FALSA recién salida de firmar. Acá no se repite el error.
  // ⛔ Tampoco dice "empezá el envío de nuevo": el permiso venció, no la remesa.
  deeplink_pop_vencido:
    "Pasó demasiado tiempo y el permiso que estábamos usando ya no vale. No mandamos nada con él. Volvé a intentarlo y te lo pedimos de nuevo.",
  // 🔴 LA RESPUESTA NO ES LA QUE PEDIMOS. ⛔ No dice "alguien te está atacando": un canal cruzado con
  // otro envío llega exactamente acá. Y ⛔ no dice "cancelaste" (eso es `deeplink_rechazado`, donde la
 ...
 * archivo de [[CENSO src/presentation/flow.tsx lineas=4453]] líneas con [[CENSO src/presentation/flow.tsx entrantes=165]] citas ANCLADAS entrantes sería expandir el scope para "cumplir" un AC. ⚠️ ACÁ DECÍA «4268 líneas con 83 citas» Y LO VOLVIÓ FALSO EL COMMIT `7338c37` DE ESTA MISMA HU, sin que nadie editara la frase (F4/F4-2). Por eso hoy son MARCADORES que verifica `citas-ancladas.test.ts` contra el árbol en cada `npm test`, no cifras escritas a mano. ⚠️ Y «ANCLADAS» no es un adorno: el marcador cuenta SÓLO las citas con símbolo delante — las sueltas de (`resolverVueltaDelPermiso`, `flow.tsx:4009`) se rompen igual y siguen sin que las mire nadie.
 ...
 *      (`POP_SECRET`, `../../app/api/payout/prepare/route.ts:216`) exige el PoP, P1..P5 lo verifican ed25519 contra el
 *      `sender`, y (`prepare_kyc_verdict_missing`, `../../app/api/payout/prepare/route.ts:371`) exige la fila del veredicto SIN respaldo. Ninguna remesa deposita sin que el
 ...
 *  `escrowFundsKnowledge` certifica: que el depósito no salió. ⚠️ Y LA ACCIÓN QUE SUGIERE TIENE QUE SERVIRLE A LOS OCHO, que es lo que no pasaba (re-AR it3 · MNR-4). Decía «Podés empezar de nuevo con una cotización fresca» a secas, y para `prepare_rate_limited` hacerlo INMEDIATAMENTE vuelve a chocar con el mismo límite, mientras que para `prepare_not_configured` —una misconfiguración NUESTRA— reintentar no funciona nunca. No era falsa; era una acción que a 2 de sus 8 enums no les sirve. El «en un rato» sale del vocabulario que este archivo ya usa (`payout_authority_unavailable`, `:750`), y el cierre dice que el problema no es de la persona sin prometer que reintentar lo arregla. ⛔ NO SE PARTE POR ENUM: una lista de casos acá es el patrón que (`copyDeEntregaFallida`, `:1602`) vino a eliminar, y volvería a envejecer con el enum 15. ⚠️ Y NOMBRA LA FIRMA, por consistencia con su hermano (`payout_authority_unavailable`, `:750`), que la nombra a propósito porque callarla induce a error. La precondición está MEDIDA, no supuesta: el gateway NUNCA postea sin PoP (`prove`, `../infrastructure/settlement/http-solana-prepare-gateway.ts:290` — sin prueba corta con `payout_pop_unavailable` y no hay POST), así que si la pantalla llegó a leer uno de estos 8 enums, la billetera firmó. La frase describe PARA QUÉ sirve esa firma, no que haya verificado bien: así también es cierta para `payout_pop_unverified`, que es justo el caso en que no verificó. */
export const COPY_FALLO_SIN_DEPOSITO =
  "No se pudo entregar y no llegó a salir ningún USDC de tu wallet: no hay ningún reembolso pendiente ni nada que reclamar. La firma que te pedimos no mueve plata: sólo sirve para probar que la billetera es tuya. Probá de nuevo en un rato con una cotización fresca; si te sigue pasando, el problema es nuestro y no hay nada que puedas arreglar desde tu lado.";
...
// entrega fallida (`COPY_FALLO_SIN_DEPOSITO`, `:1599`).

/**
 * Cómo TERMINÓ el bucle que retoma la verificación. `null` = no terminó (o nunca corrió).
 *
 * 🔴 ES UN MOTIVO Y NO UN BOOLEANO, y no es ceremonia: el booleano que esto reemplaza colapsaba dos
 * finales que no comparten ni un hecho. `"sin-respuesta-en-el-techo"` es *preguntamos varias veces y
 * ninguna respuesta fue final*; `"no-pudimos-preguntar"` es *el resume se cortó y la pantalla no llegó a mostrar cómo quedó*. ⛔ NINGUNO DE LOS DOS ENUNCIA SOBRE SI LA CONSULTA SALIÓ (CR/BLQ-BAJO-1). Acá vivía la hermana de la frase que el fix-pack ya sacó del copy y de (`finDelResume`, `flow.tsx:171`) —«no sabemos ni si la consulta salió»— y era falsa por el mismo motivo: con el throw en (`kycStore.save`, `resume-kyc.ts:49`) o en (`pending.clear`, `resume-kyc.ts:50`) la consulta SALIÓ, contestó, y (`applyKyc`, `resume-kyc.ts:47`) y (`repo.save`, `resume-kyc.ts:48`) YA persistieron el veredicto. ⛔ Afirmar de más en la DEFINICIÓN es peor que en el copy: es de acá que el próximo saca la frase que después va a la pantalla.
 * ⚠️ Y EL NOMBRE `"no-pudimos-preguntar"` SIGUE SIENDO IMPRECISO POR ESE MISMO SUB-CASO. Se decide MANTENERLO, y las tres razones son verificables: (1) los dos literales están fijados textualmente por el SDD aprobado (§4.8) — cambiarlos en un fix-pack de CR es drift de spec, no un arreglo; (2) es el vocabulario que este repo ya usa para el tercer valor en (`EstadoDeLaCuentaDeNonce`, `../infrastructure/solana/preparacion-por-enlace.ts:62`), y renombrar sólo acá lo parte en dos; (3) el nombre es una ETIQUETA INTERNA y lo único que lo vuelve píxeles es (`copyDelFinDelResume`, `:1747`), que lo mapea a constantes cuyo texto sí enuncia sobre la pantalla. ⛔ AGUJERO DECLARADO Y MEDIDO: si alguien renderizara el literal crudo no lo cazaría nadie — el léxico vedado de `honest-copy.test.tsx` busca «no pudimos» CON ESPACIO y este valor lleva guiones.
 * Con un `boolean` los dos se pintaban con la MISMA card, y la card tenía que elegir una causa: eligió la del primero y se la aplicó también al segundo. Es la misma lección que
...
export type FilaDeDesenlace = {
   ...
   *  🔴 CD-10 (AR/MNR-5) · POR QUÉ CADA FILA REPITE SUS CITAS EN UN `//` DE AL LADO, Y NO ES REDUNDANCIA DECORATIVA. `citas-ancladas.test.ts` **saltea las cadenas a propósito** (su lexer: (`lineasConComentario`, `../composition/citas-ancladas.test.ts:112`)), así que una cita que vive DENTRO de este `string` no la mira nadie: las de acá abajo estaban correctas y sin candado, que es el estado del que sale una cita rota sin que nadie se entere. ⛔ ACÁ HABÍA UNA CIFRA («las 21») Y EL ÁRBOL DABA OTRA (CR/MNR-1). Se borra en vez de re-medirse, y el motivo es el criterio de CD-15: ningún instrumento la re-deriva —los marcadores `[[CENSO …]]` cuentan las citas ENTRANTES a un archivo, no las SALIENTES de un bloque— así que la próxima fila que se agregue acá abajo la vuelve falsa otra vez y en silencio. Lo que sí se puede verificar, y es lo que vale, es el invariante de al lado: cada `porQue` tiene su espejo anclado en el `//`, y el espejo lo mira `citas-ancladas.test.ts` en cada `npm test`. ⛔ NO se pudieron MOVER —que era lo barato— porque `T-073-CENSO` exige por regex que el `porQue` TENGA una cita, y sacarlas lo pone rojo en las cinco filas; aflojar ese candado no es una opción. ⇒ La cita del `porQue` es la que se LEE y la del `//` es la que se VERIFICA (anclada). Si el destino se mueve, el `//` se pone rojo **y eso es el aviso de que la del string quedó vieja**: se arreglan las dos juntas, nunca una sola. ⛔ Y EL DISPARADOR DEL REDISEÑO, QUE HASTA HOY VIVÍA SÓLO EN UN `doc/` GITIGNOREADO Y NO VIAJABA CON EL REPO (F4/F4-3): la SEXTA fila de `DESENLACES_DEL_RESUME`. Con cinco, mantener el espejo a mano cuesta cero; en la sexta, `{ texto, citas[] }` —con el censo RESOLVIENDO cada cita en vez de exigirla por regex— se vuelve más barato que seguir duplicándolas. La otra mitad del disparador es la frase de acá arriba y ⛔ no se repite. */
