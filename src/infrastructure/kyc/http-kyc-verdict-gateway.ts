...
const ABSENT_REASONS = ["absent", "expired", "not_approved"] as const; // EN ESTA LÍNEA: este archivo recibe SEIS citas por número —a `:19-24` desde dos sitios, a ESTA misma línea (anclada, desde `app/api/kyc/verdict/route.ts`), y a `:57`, `:61` y `:74`— y una línea de más las corre a todas. ⚠️ ACÁ DECÍA que la cita a proteger era «`solana-wallet.ts:2374` cita `:60`», y hace un commit que dejó de ser cierto (CR/MNR-3): el fix-pack del AR re-derivó ese emisor a `:74` y lo ancló, y `:60` hoy es una línea EN BLANCO. La nota protegía un número que ya no citaba nadie. WKH-233 — `"simulated"` se fue de `KycVerdictAbsentReason` y por eso se va de acá: la fila del veredicto se escribe SÓLO con `payoutAllowed === true` del agente de KYC, así que una fila que existe es real por invariante y ningún servidor puede emitir ya ese motivo. Un servidor viejo que lo emitiera cae en la rama de `readAbsentReason` y cuenta como `absent`, que es el fail-safe correcto: la respuesta existe, pero este cliente no puede afirmar de qué motivo habla
