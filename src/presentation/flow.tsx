...
import {
  ...
  cruceDeCuenta, seVerificoLaCuenta, copyDeEntregaFallida, COPY_RESUME_NO_PODEMOS_SEGUIR, COPY_RESUME_NO_PODEMOS_SEGUIR_AVISO, LABEL_VOLVER_A_EMPEZAR_EL_ENVIO, copyDelFinDelResume, type FinDelResume } from "./flow-vm"; // WKH-359/AC-6 — los dos EN ESTA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=104]] citas ancladas y una línea nueva acá arriba las corre a todas — el número es un MARCADOR verificado por `citas-ancladas.test.ts`, no una cifra escrita a mano: cuando esta HU lo escribió decía 83 y su propio commit lo volvió falso). ⚠️ Van ACÁ y no al final de `:74` porque esa línea TERMINA en comentario: pegar un import ahí lo deja comentado, y `tsc` no lo caza hasta que algo lo use. Me pasó dos veces en esta HU.
import { cn } from "./cn";
...
export function RemittanceFlow({ container, pasoInicial = "bienvenida" }: { container?: Container; pasoInicial?: Step } = {}) {
  ...
  const adoptarCuentaConectada = () =>
    guard(async () => {
      ...
      setAddress(r.address); if (r.estado === "hay-que-salir") { window.location.href = r.irA; return; } // WKH-359/AC-3 — EN ESTA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=104]] citas ancladas, marcador verificado). La dirección se aplica IGUAL antes de salir, porque el `connect()` ya corrió y ya la conocemos: perderla obligaría a re-conectar al volver del salto
