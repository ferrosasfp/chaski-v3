...
//
// Este archivo existe para matar UN mutante concreto (M10): cambiar un byte del builder **de un solo
// repo** — por ejemplo `network:` → `Network:`. Un mutante así no rompe nada localmente (el repo
// mutado sigue siendo internamente coherente) y sólo se manifiesta cuando una billetera real firma
// de un lado y un servidor real verifica del otro. En producción eso se ve como "el patrocinio dejó
// de andar", sin ninguna pista de dónde mirar.
//
// ⚠️ ESTE ARCHIVO ESTÁ DUPLICADO A PROPÓSITO. Su gemelo es
// `wasiai-facilitator/src/__tests__/fixtures/sponsor-pop-golden.ts` y tiene el MISMO contenido byte a
// byte. La duplicación no es un descuido: son dos repos que no comparten paquete, y un vector
// compartido por import dejaría de detectar la divergencia que vino a detectar.
//
// ⚠️ `expectedMessage` es un LITERAL escrito a mano (CD-9). PROHIBIDO reemplazarlo por una llamada a
// `buildSponsorPopMessage`: esa aserción se mueve junto con el mutante y pasa siempre.
//
...
//   5. pegar el resultado ACÁ y en el gemelo del facilitator, y correr `T-G1` en los dos repos.
export const GOLDEN = {
  ...
  /** USDC devnet de Circle (`src/infrastructure/chain.ts:19`). */
