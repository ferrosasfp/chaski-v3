...
describe("WKH-351 · la tarjeta del historial no muestra la etiqueta del trámite", () => {
  ...
  // MUTANTE (b): restaurar el Pill en `flow.tsx:3246`, o poner cualquier otra etiqueta con forma de
  // Pill y texto nuevo.
  ...
  it("T-N1: en los 4 grupos está el encabezado y no hay ninguna etiqueta de trámite", async () => {
    ...
    for (const g of LOS_4_GRUPOS) {
      ...
      // en `flow.tsx:3246` la tarjeta decía "Estado: Pago en curso" bajo "Necesitan tu firma" —el caso
  ...
  // `payout_submitted`; revertir sólo el hunk de `flow.tsx:3246` dejando el de `flow.tsx:3226`; o mover la
  ...
  // gap-3">` de `flow.tsx:3239` junto con el Pill ("un flex con un solo hijo sobra"), que se lleva
  // puestos el nombre, el monto y la fecha. O borrar el bloque `flow.tsx:3239-3247` entero. Además, esas 4
  ...
  // `formatEntryDate` (`flow.tsx:3268`) devuelve cuando el `createdAt` es implanteable.
