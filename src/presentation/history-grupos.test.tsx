...
describe("WKH-351 · la tarjeta del historial no muestra la etiqueta del trámite", () => {
  ...
  // MUTANTE (b): restaurar el Pill en `flow.tsx:3307`, o poner cualquier otra etiqueta con forma de
  // Pill y texto nuevo.
  ...
  it("T-N1: en los 4 grupos está el encabezado y no hay ninguna etiqueta de trámite", async () => {
    ...
    for (const g of LOS_4_GRUPOS) {
      ...
      // (`flow.tsx:679-680`) también es un `span.rounded-full`, y un selector global lo contaría.
      ...
      // en `flow.tsx:3307` la tarjeta decía "Estado: Pago en curso" bajo "Necesitan tu firma" —el caso
  ...
  // `payout_submitted`; revertir sólo el hunk de `flow.tsx:3307` dejando el de `flow.tsx:3287`; o mover la
  ...
  // NO DUPLICA: (`statusDisplay`, `flow-vm.test.ts:76-101`) candadea el PURO y `flow.test.tsx:1548-1558` cubre 2 de
  ...
  // gap-normal">` de `flow.tsx:3300` junto con el Pill ("un flex con un solo hijo sobra"), que se lleva
  // puestos el nombre, el monto y la fecha. O borrar el bloque `flow.tsx:3300-3308` entero. Además, esas 4
  ...
  // `formatEntryDate` (`flow.tsx:3329`) devuelve cuando el `createdAt` es implanteable.
