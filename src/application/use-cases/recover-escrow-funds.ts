 ...
 *  vault. `escrowFundsKnowledge` ya lo clasificaba como `unverified` (`escrowFundsKnowledge`, `flow-vm.ts:190`), o sea que el
...
export class RecoverEscrowFunds {
  ...
  async execute(input: { remittanceId: string; sender: string }): Promise<RecoverEscrowFundsResult> {
    ...
    try {
      ...
      // después (`onOpenFromHistory`, `flow.tsx:340`) — por eso ahora NO arranca sobre un estado que ya no avanza solo.
    }
