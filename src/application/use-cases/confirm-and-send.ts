...
export class ConfirmAndSend {
  ...
  async execute(input: { remittanceId: string; hrefDeLaVuelta: string }): Promise<ResultadoDeEnvio> { // 🔴 WKH-373 — EL SEGUNDO CAMPO, EN ESTA MISMA LÍNEA (Δ0: este archivo tiene censos anclados a `:463`). ⛔ SIN `?`: es el href de la barra TAL COMO ESTABA AL MONTAR, y su único destino es `authorizePrincipal` (`:486`), que hasta WKH-373 lo leía de `globalThis.location` EN VIVO — o sea, ya limpiado por `limpiarLaBarra` (`../../presentation/flow.tsx:4023`). Este caso de uso ⛔ NO lo interpreta, ⛔ no lo parsea y ⛔ no lo reescribe: lo pasa tal cual, que es lo que hace que la aplicación NO dependa del orden entre dos archivos.
    ...
    //    HOY los tres tienen rama propia, y las citas van ANCLADAS para que `src/composition/citas-ancladas.test.ts` se ponga rojo si alguna se muda: (`payout_not_authorized`, `../../presentation/flow-vm.ts:748`), (`prepare_kyc_verdict_missing`, `../../presentation/flow-vm.ts:695`) y (`payout_authority_unavailable`, `../../presentation/flow-vm.ts:750`). El CONTENIDO de cada copy lo miden T-COPY-2, T-COPY-1 y T-COPY-5 en `../../presentation/flow-vm.test.ts`.
    //    ⚠️ ACÁ DECÍA «lo que NADA DE ESO MIDE es que esta lista de tres sean TODOS los enums que `prepare` puede devolver … si mañana aparece un cuarto, esta frase vuelve a ser dos de tres sin que ningún test cambie de color». **No hacía falta mañana: ya eran doce** (re-AR it2 · BLQ-MED-1). La route emite 14 enums y 12 llegaban al catch-all del escrow, porque el copy propio no bastaba: la PANTALLA no llamaba a `humanError` con el reason real. ⇒ HOY ESTA LISTA YA NO ES LO QUE SOSTIENE LA AFIRMACIÓN, y por eso deja de envejecer: la pantalla elige la frase por el ESTADO DEL DEPÓSITO, no por el enum ((`copyDeEntregaFallida`, `../../presentation/flow-vm.ts:1602`)), así que un enum nuevo entra al mecanismo sin que nadie lo declare. El barrido que lo re-mide en cada `npm test` deriva el conjunto de enums LEYENDO la route: `src/presentation/copy-de-prepare-en-pantalla.test.tsx` (T-PANT-2). ⛔ Lo que ese barrido NO mide, declarado allá: el salto `HTTP status ⇒ enum` del `switch` de la route.
    //
