...
export function humanError(code: string): string {
  ...
  // `payout_failed` cuyo reason no reconozca (`humanError`, `flow.tsx:1697`), justo cuando no sabemos dónde está la
 ...
 *  USDC.", `flow.tsx:2588`), aunque nombre la misma acción. Mi primera versión la copiaba tal cual para
 ...
 * que no lleva a ningún refund: `settled` no está en (`refundeable`, `flow.tsx:1501`), que es
 ...
 * (`idsAConsultar`, `flow.tsx:3173`), y (`escrowOutcome`, `:1193`) devuelve el conocimiento local tal cual con `"not-asked"`)
