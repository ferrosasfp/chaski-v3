...
/** La MISMA derivación que `deriveEscrowState`, `solana-wallet.ts:277`. */
...
describe("SolanaWalletAdapter.closeEscrow (WKH-327)", () => {
  ...
  it("AC-5: confirmación limpia + la cuenta SIGUE AHÍ ⇒ 'pending', NUNCA 'confirmed' (M4)", async () => {
    ...
    // tx confirma sin error (`confirmRefund`, `solana-wallet.ts:1448`), SIN leer nada. AC-5 exige leer la AUSENCIA. Un
