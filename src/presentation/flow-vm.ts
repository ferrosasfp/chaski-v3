...
export function humanError(code: string): string {
  ...
  // `payout_failed` cuyo reason no reconozca (`humanError`, `flow.tsx:1643`), justo cuando no sabemos dónde está la
 ...
 *     la garantía está UN SALTO ANTES: (`onOpenFromHistory`, `./flow.tsx:413`) sólo
 ...
 *
 *  🔴 Y LA EVIDENCIA CORREGIDA (CR/MNR-1), porque mi primera corrección citaba mal: la garantía está UN SALTO ANTES: (`onOpenFromHistory`, `./flow.tsx:413`) sólo
 *     hace `setError`/`setRem`/`setStep` —**no toca la address**—, pero a la pantalla de historial se
 *     llega únicamente por `openHistory`, que hace `await resolveSender()` (y ése sí hace `setAddress`)
 *     ANTES de `setStep("history")`. El otro sitio (`:427`, tras confirmar) llega con la wallet ya
 *     conectada. */
 ...
 *  USDC.", `flow.tsx:2534`), aunque nombre la misma acción. Mi primera versión la copiaba tal cual para
 ...
 * Un "se cerró" ahí le diría a alguien que no hay nada que recuperar sobre una fila donde SÍ lo hay, y le apagaría (`LostEscrowRecovery`, `flow.tsx:865`), que es la puerta que le queda. Por eso el copy dice lo MEDIDO y deja abierta la tercera posibilidad. Candado: T-W10. R-1 en (`EscrowChainState`, `../application/ports.ts:1122`). 🔴 Y EL COPY DECÍA DOS COSAS QUE ESTE MISMO DOCBLOCK REFUTA (AR r2 · BLQ-BAJO-1), corregidas acá. (1) Decía "en EL CONTRATO que estamos consultando no figura ninguna cuenta", que generaliza de UNA lectura a todo el programa: lo que se lee es una sola dirección, la que (`deriveEscrowStateFromId16`, `../infrastructure/solana-wallet.ts:294`) deriva de `["escrow", sender, id16]`. Ahora dice la dirección de este envío y que miramos esa sola. (2) La tercera posibilidad decía "siga abierta en un contrato que no estamos mirando", y ése es justo el lugar donde el disparador MÁS PLAUSIBLE de los cuatro de arriba NO la pone: si la fila quedó con un `ownerAddress` y el depósito lo firmó otra cuenta de la wallet, la PDA existe en ESTE MISMO programa, sembrada con el otro sender, y (`LostEscrowRecovery`, `flow.tsx:865`) la encuentra porque resuelve por sender. Por eso el copy ahora nombra la otra dirección y manda a esa puerta, en vez de mandar a un contrato que nadie va a ir a buscar. ⚠️ RESIDUO DE ESA ÚLTIMA FRASE: nombra la puerta en prosa ("recuperar un envío perdido"), no por su label. Si alguien renombra el label de esa puerta, que hoy dice "Recuperar un envío perdido" en el cuerpo de (`LostEscrowRecovery`, `flow.tsx:2146`), esta frase queda apuntando a algo que en pantalla se llama de otra forma. ⚠️ ESO CAMBIÓ A MEDIAS: T-W12 (el candado de acá abajo) busca ese botón POR SU LABEL, así que un renombre lo pone rojo y obliga a mirar esta frase; lo que sigue sin vigilancia es que la PROSA del copy y el label digan LO MISMO, porque nadie los compara. Duplicar el label byte a byte cambiaría un riesgo por el otro, el de las dos copias que divergen. 🔴 EL CONSEJO NO SE PODÍA CUMPLIR DESDE DONDE SE LEE (AR r3 · BLQ-MED-1), Y ESO SE CERRÓ EN DOS ETAPAS. Decía "conectá esa y usá la opción de recuperar un envío perdido", y eso ERA inaplicable en la sesión en la que se lee: `address` es un `useState` (`address`, `flow.tsx:147`) y (`resolveSender`, `flow.tsx:387`) devolvía el valor local cuando ya lo tenía —un `address ??` que cacheaba la primera cuenta para siempre—, así que nunca se le volvía a preguntar a la billetera. Y esta tarjeta SÓLO puede verse con `address != null`, por construcción: (`HistoryView`, `flow.tsx:1094`) recibe `sender={address}`, su efecto corta sin sender (`reader`, `flow.tsx:3136`) y sin consulta la respuesta es `not-asked`, que devuelve el conocimiento local (`answer`, `:1196`). O sea: LA MISMA CONDICIÓN QUE HACÍA VISIBLE EL CONSEJO ERA LA QUE LO VOLVÍA IMPOSIBLE. Lo único que limpiaba `address` era (`forgetAndDisconnect`, `flow.tsx:550`), detrás de "Borrar igual", que además borra el KYC y las entries del owner. ETAPA 1 (WKH-352): el copy pasó a mandar a volver a abrir la app, porque el montaje nuevo arranca con `address` en `null`. ETAPA 2 (WKH-354), que es la que vale hoy: se ELIMINÓ el `address ??`, y (`resolveSender`, `flow.tsx:387`) le pregunta al puerto `connectedWallet` —la billetera VIVA— en cada una de las tres puertas no monetarias, así que la reapertura dejó de hacer falta y el copy dejó de pedirla. Acá decía "⛔ NO se agregó ningún control de cambiar billetera ni se tocó `resolveSender`: eso cambia la conexión para TODA la app y es otra HU": esa HU fue WKH-354 y esa frase ya es falsa, por eso se reescribió. Y el arreglo NO es esta prosa: es T-354-W12 + su control en `history-onchain.test.tsx`, que ejecutan el consejo de punta a punta —sin reabrir la app— y assertan con qué `sender` sale la recuperación. Tres rondas de AR se fueron en reescribir texto que prometía de más; un texto no se puede medir a sí mismo.
 *
 ...
 * que no lleva a ningún refund: `settled` no está en (`refundeable`, `flow.tsx:1447`), que es
 ...
 * `flow.tsx:865`), en la pantalla de inicio. No mira el estado local de la remesa: resuelve el id
 ...
 * (`idsAConsultar`, `flow.tsx:3119`), y (`escrowOutcome`, `:1193`) devuelve el conocimiento local tal cual con `"not-asked"`)
