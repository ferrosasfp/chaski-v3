#!/usr/bin/env node
/**
 * Aplica a bdwv la columna de proveniencia del desembolso:
 *   supabase/migrations/20260804T000000_add_payout_provenance_to_remittance_settlements.sql
 *
 * ⚠️ EL ORDEN IMPORTA Y NO ES SIMÉTRICO. Esta migración va **ANTES** de desplegar el
 * código que escribe la columna. Al revés, el upsert de `recordOrderPrepared` nombra una
 * columna inexistente (PGRST204), la función tira, y como el write es best-effort (CD-17)
 * el prepare igual responde 200: se pierde en silencio la fila ENTERA de evidencia de cada
 * orden preparada en esa ventana. Y la columna también está en el SELECT, así que las
 * lecturas admin tiran y /api/admin/reconcile-orphans responde 503.
 *
 ...
 *
 * Uso:  SUPABASE_ACCESS_TOKEN=... node scripts/apply-payout-provenance-migration.mjs [--dry-run]
 */
...
const CALDZ_REF = 'caldzjhjgctpgodldqav'; // PROHIBIDO (mainnet, archivo)
const TARGET_REF = BDWV_REF;

const TABLE = 'remittance_settlements';
const COLUMN = 'payout_provenance';
...
// ── Guard 3: confirmar que la base responde ────────────────────────────────
...
// ── PRE-ESTADO ─────────────────────────────────────────────────────────────
// Sólo se consulta lo que existe ANTES de la migración. Preguntar por la columna
// nueva en el pre-estado tira 42703 y aborta por una causa falsa (lo cazó un --dry-run
// anterior en otro repo).
const PRE_SQL = `
SELECT
  (SELECT count(*) FROM information_schema.tables
     WHERE table_schema = 'public' AND table_name = '${TABLE}')      AS tabla_existe,
  (SELECT count(*) FROM information_schema.columns
     WHERE table_name = '${TABLE}' AND column_name = '${COLUMN}')    AS ya_tiene_columna,
  (SELECT count(*) FROM public.${TABLE})                             AS filas;`;
...
if (!pre.text.includes('"tabla_existe":1')) {
  console.error(`[ABORT] La tabla ${TABLE} no existe en el destino. Base equivocada.`);
  process.exit(3);
}
...
console.log(`\n[post-estado] la columna:\n${post.text}`);

const counts = await query(PRE_SQL);
console.log(`\n[post-estado] conteos:\n${counts.text}`);

const okCol = post.ok && post.text.includes(COLUMN);
const okNullable = post.ok && post.text.includes('"is_nullable":"YES"');
console.log('\n── VERIFICACIÓN ──');
console.log(`  la columna ${COLUMN} existe   ${okCol ? '✅' : '❌'}`);
console.log(`  es nullable (filas viejas)     ${okNullable ? '✅' : '❌'}`);
if (!okCol || !okNullable) {
  console.error('\n[FAIL] El post-estado no es el esperado.');
  process.exit(1);
}
console.log('\n[OK] Migración aplicada y verificada contra bdwv.');
console.log('     AHORA SÍ se puede mergear/desplegar el código que escribe la columna.');
