...
import {
  ...
  cruceDeCuenta, seVerificoLaCuenta, copyDeEntregaFallida, COPY_VERIFY_PIDE_UNA_NUEVA, COPY_RESUME_NO_PODEMOS_SEGUIR, COPY_RESUME_NO_PODEMOS_SEGUIR_AVISO, LABEL_VOLVER_A_EMPEZAR_EL_ENVIO, copyDelFinDelResume, type FinDelResume } from "./flow-vm"; // WKH-359/AC-6 — los dos EN ESTA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=165]] citas ancladas y una línea nueva acá arriba las corre a todas — el número es un MARCADOR verificado por `citas-ancladas.test.ts`, no una cifra escrita a mano: cuando esta HU lo escribió decía 83 y su propio commit lo volvió falso). ⚠️ Van ACÁ y no al final de `:74` porque esa línea TERMINA en comentario: pegar un import ahí lo deja comentado, y `tsc` no lo caza hasta que algo lo use. Me pasó dos veces en esta HU.
import { cn } from "./cn"; import { anotarCorteDeVuelta, anotarHito, anotarLaSalidaAlNavegador, type BorradorEnElDisco, formaDelDestino } from "./bitacora-de-vuelta"; // HU-075/diagnóstico — EN ESTA LÍNEA y no en una nueva: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=165]] citas ancladas y una línea de import de más las corre a todas. HU-075/gesto: `formaDelDestino` PEGADO ACÁ por lo mismo
import { useWalletAvailability, useConnectedWalletAddress, mwaEnabled, useMwaOffered, deeplinkEnabled } from "./wallet-availability"; import type { BilleteraDeeplink } from "../infrastructure/solana/deeplink/protocol"; import { hrefSinRastroDeVuelta, marcaDeVuelta } from "../infrastructure/solana/deeplink/conexion"; import { formaDelHref } from "../infrastructure/solana/deeplink/forma-del-href"; /* WKH-373: EN ESTA MISMA LÍNEA (Δ0). ⛔ COMENTARIO DE BLOQUE Y NO `//`: esta línea sigue con más imports a la derecha. */ import type { EstadoDeLaCuentaDeNonce } from "../composition/container"; import { NONCE_ACCOUNT_RENT_LAMPORTS, formatLamportsAsSol } from "../application/solana-escrow-rent"; import { esperarDisponibilidadDecidible } from "../infrastructure/solana/disponibilidad-decidible"; import { MARCAS_DE_VUELTA } from "../infrastructure/solana/deeplink/sesion"; import { DEEPLINK_MARCA_SIN_CONSUMIDOR, DEEPLINK_NONCE_AUSENTE } from "../infrastructure/solana/deeplink/firma-por-enlace"; import { URL_INSTALAR_PHANTOM, hrefSinLaMarcaDeSalida, urlDeSalidaAlNavegadorDeLaBilletera, vinoDeUnaSalidaConBorrador } from "./salida-al-navegador-de-la-billetera"; import { KEY as CLAVE_DEL_REPO } from "../infrastructure/persistence"; // WKH-372/W1 — LOS DOS IMPORTS EN ESTA MISMA LINEA (Δ0) y ANTES de este `//`, por lo que este mismo renglon ya explica mas abajo: pegarlos DESPUES del comentario los deja COMENTADOS sin que `tsc` los cace. // WKH-075 — LOS TRES EN ESTA MISMA LÍNEA (Δ0), y ANTES de este comentario y no después: `:77` termina en comentario igual que `:74`, y pegar un import al final del renglón lo dejaría COMENTADO sin que `tsc` lo cace hasta que algo lo use. Lo que hace a `:77` la línea correcta es que es una cadena de `import ...;` COMPLETOS, así que existe un punto de inserción; `:74` está dentro de una lista con llaves que sigue en `:75` y ahí no existe. // WKH-358: los dos EN ESTA MISMA LÍNEA, por lo mismo que los dos de WKH-MWA // el aviso de "acá no hay wallet" (NoWalletHere) · WKH-354/AC-6: `useConnectedWalletAddress` para el banner (CuentaCambiada) · WKH-MWA: los dos últimos, EN ESTA MISMA LÍNEA (el censo de citas por número de `:44` `flow.tsx:NNNN` cuelgan de que este archivo no cambie de largo)
...
export function RemittanceFlow({ container, pasoInicial = "bienvenida" }: { container?: Container; pasoInicial?: Step } = {}) {
  ...
  const [step, setStep] = useState<Step>(pasoInicial); const [aterrizaje] = useState<{ vinoConMarca: boolean; borradorEnElDisco: BorradorEnElDisco }>(() => { if (typeof window === "undefined") return { vinoConMarca: false, borradorEnElDisco: "disco-ilegible" }; let borradorEnElDisco: BorradorEnElDisco; try { const crudo = window.localStorage.getItem(CLAVE_DEL_REPO); const filas: unknown = crudo === null ? [] : JSON.parse(crudo); borradorEnElDisco = Array.isArray(filas) && filas.length > 0 ? "con-borrador" : "sin-borrador"; } catch { borradorEnElDisco = "disco-ilegible"; } return { vinoConMarca: vinoDeUnaSalidaConBorrador(window.location.href), borradorEnElDisco }; }); useEffect(() => { anotarHito("pantalla", step); anotarLaSalidaAlNavegador(aterrizaje); const sinLaMarca = typeof window === "undefined" ? null : hrefSinLaMarcaDeSalida(window.location.href); if (sinLaMarca !== null) window.history.replaceState(window.history.state, "", sinLaMarca); }, [step, aterrizaje]); // HU-075/diagnóstico it2 — EL HITO DE LA PANTALLA, EN ESTA MISMA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=165]] citas ancladas por número y mide [[CENSO src/presentation/flow.tsx lineas=4453]] líneas; una línea nueva las corre a todas). 🔴 POR QUÉ HACE FALTA: el bloque de diagnóstico es HERMANO de esta pantalla en `app/page.tsx` y ⛔ no puede leer un solo `useState` de acá, así que «estoy en la bienvenida» era una descripción de una foto y no un dato — y es contra ese dato que se lee el desenlace de `onConnect`. ⛔ VA EN UN EFECTO Y NO EN EL RENDER: escribir un store de módulo durante el render es impuro y bajo StrictMode se ejecutaría dos veces. ⛔ Y NO NOTIFICA A NADIE: (`anotarHito`, `./bitacora-de-vuelta.ts:101`) es un `Map.set` sin oyentes, así que esto no re-renderiza nada, ni acá ni en el hermano — el bloque refresca solo 🔴 WKH-372/W1 · EL ATERRIZAJE, LOS DOS EN ESTA MISMA LINEA (Δ0). QUE ES: la foto de con que se encontro la app AL LLEGAR, tomada UNA vez en el inicializador del estado — el mismo patron, y por el mismo motivo, que `vueltaSinResolver` en `:163`: leerlo en un efecto lo mediria DESPUES de que la pantalla haya podido escribir el disco, o sea que se estaria midiendo el resultado de la propia medicion. ⚠️ EL DISCO SE LEE ACA Y UNA SOLA VEZ, y de aca lo toman los DOS consumidores: el aviso de aterrizaje de `:757` y el hito que publica el bloque de diagnostico. Tenerlo en dos lados serian dos lecturas del mismo disco que se corrigen por separado. 🔴 CR/`BLQ-BAJO-1` — Y LA RESPUESTA DEL DISCO TIENE TRES VALORES, NO DOS. Aca habia un `catch { hayBorrador = false }`, o sea «no pude leer el disco» publicado como «no hay borrador»: con el borrador PUESTO y un `getItem` que tira, la pantalla le decia a la persona que sus datos no estaban teniendolos ahi. El tercer valor es (`BorradorEnElDisco`, `./bitacora-de-vuelta.ts:168`) y ⛔ su nombre no se invento aca: `disco-ilegible` es el que (`MotivoParaNoMostrar`, `./splash-puerta.ts:68`) ya usa para este mismo hecho. ⚠️ SIN `window` TAMBIEN ES `disco-ilegible` y no `sin-borrador`, por lo mismo: en el servidor no se pregunto nada. No es observable —sin `window` la marca es `false` y el hito sale `sin-marca` igual—, pero la foto no afirma lo que no midio. ⛔ `anotarLaSalidaAlNavegador` se anota una unica vez —lo garantiza ELLA, no este efecto—, asi que el `[step]` de las dependencias no la re-escribe en cada cambio de pantalla. Lo mide `T-372-W1-7` en `wallet-availability.test.tsx`, y borrar esta llamada lo pone rojo 🔴 CR/`BLQ-MEDIO-1` · Y ACA SE CONSUME LA MARCA `wb`, QUE ES LO QUE HACE HONESTO AL INSTRUMENTO. Nadie la borraba de la barra, y el CR lo midio: aterrizar con `?wb=1` y el disco vacio da aviso puesto e hito `con-marca-sin-borrador` («no cruzo»); si la persona re-tipea y RECARGA la pestaña con `?wb=1` todavia en la barra, los dos instrumentos pasan a publicar `con-marca-y-borrador` («cruzo») sobre datos que ella cargo a mano. Un pull-to-refresh invertia la medicion en silencio. ⛔ VA EN EL EFECTO Y NO EN EL INICIALIZADOR DE `useState`: escribir la barra durante el render es impuro, y bajo `StrictMode` el inicializador corre DOS veces, asi que la segunda pasada leeria una URL que la primera ya limpio y las dos fotos no coincidirian. Aca el efecto tambien corre dos veces bajo `StrictMode` y no pasa nada: `aterrizaje` ya esta congelado, y (`hrefSinLaMarcaDeSalida`, `./salida-al-navegador-de-la-billetera.ts:144`) devuelve `null` en la segunda porque ya no hay marca ⇒ ni un `replaceState` de mas. ⛔ Y NO ROMPE EL ATERRIZAJE LEGITIMO: ni el aviso ni el hito leen la URL, los dos salen de la foto de esta linea. Lo mide `T-372-W1-7e`
  ...
  const [address, setAddress] = useState<string | null>(null); const [saltoPendiente, setSaltoPendiente] = useState<string | null>(null); /* HU-075/gesto — EL TERCERO EN ESTA MISMA LÍNEA (Δ0, ver `:44`). QUÉ ES: «la vuelta resolvió que hace falta UN SALTO MÁS a la billetera, y ese salto todavía no salió». ⛔ NO ES UNA PERILLA NI UNA TERCERA FUENTE DE «HAY BILLETERA»: no decide nada, sólo guarda el `irA` que el desenlace ya traía, para que la pantalla pueda ofrecerlo como un `<a href>` en vez de navegar sola desde un efecto. `null` = no hay ningún salto pendiente, que es el caso de los ~100 `it` que montan esta pantalla sin marca. Se limpia solo: tocar el enlace navega de verdad y al volver el árbol se remonta. */ const [vueltaSinResolver, setVueltaSinResolver] = useState(() => typeof window !== "undefined" && esVueltaPorEnlaceNuestra(window.location.href)); // HU-075/reanudar — EL SEGUNDO EN ESTA MISMA LINEA (Δ0: este archivo recibe citas ancladas por decenas, ver `:44`). QUE SIGNIFICA: «la barra traia una marca NUESTRA al montar y el consumidor de la vuelta todavia no termino», o sea LA VENTANA en la que no se sabe si hay billetera. ⛔ NO ES UNA TERCERA FUENTE DE «HAY BILLETERA»: esas siguen siendo dos (este mismo `address` y el bridge de la inyectada) y ninguna se toca. 🔴 EL INICIALIZADOR CORRE EN EL PRIMER RENDER Y ESO NO ES UN DETALLE: la limpieza de la barra (`:4023`) borra el `dl` con `replaceState` para TODA vuelta, asi que leerlo en un efecto contestaria `false` siempre, incluidas las vueltas que si funcionaron. Lo apaga el `alResolverseLaVuelta` de `:286`, que corre pase lo que pase (el `.finally` de `:4092`). ⚠️ Sin marca en la barra arranca en `false`, asi que los ~100 `it` que montan esta pantalla sin marca quedan byte-identicos.
  ...
  }, [c]); useVueltaPorEnlace({ c, yaInteractuo: yaInteractuoRef, alConectar: (r, remittanceId) => { if (r.estado !== "listo") { anotarHito("connect", `${r.estado} · destino ${formaDelDestino(r.irA)}`); setAddress(r.address); setSaltoPendiente(r.irA); return; } anotarHito("connect", r.estado); setAddress(r.address); setServerVerdict(r.serverVerdict); setKycProof(r.kycProof); if (remittanceId !== undefined) void onConnect({ remittanceId, rc: r }); }, alResolverseLaVuelta: () => setVueltaSinResolver(false), alFallar: (causa) => { anotarCorteDeVuelta(causa); setError({ message: humanError(causa) }); }, alReanudar: (r) => { if (r.estado === "hay-que-salir") { setSaltoPendiente(r.irA); return; } setRem(r.remesa.snapshot); setStep(r.remesa.status === "settled" ? "done" : "track"); }, alAvisar: (m) => setError({ message: m }), alSaberDelNonce: setEstadoNonce }); // WKH-358/AC-1+AC-3+AC-5 — EN ESTA LÍNEA (Δ0 de citas, `:44`). El cuerpo vive al FINAL del archivo, con todo su razonamiento, por lo que dice el comentario de `:44`: ~60 líneas acá adentro corren las 131 citas por número que este archivo recibe. ⛔ La causa se traduce con `humanError` y NUNCA se escribe una causa del enlace como literal en este archivo (el candado de copy de `deeplink-callers.test.ts` cuenta los que aparecen en `src/presentation`). ⛔ Y NO hay ningún `setRem`: el gate del pisón vive adentro del hook y esto sólo repuebla lo que `onConnect` repuebla // 🔴 HU-075/gesto — LOS DOS `window.location.href` DE ESTA LÍNEA SE FUERON, Y ES EL ARREGLO ENTERO. Acá decía `if (r.estado !== "listo") { window.location.href = r.irA; return; }` en `alConectar` y lo mismo en `alReanudar`. MEDIDO en el teléfono del founder con el bloque `?diag=1` (foto t=12830 ms): `marca al montar: conectar` · `connect: hay-que-salir` · `continuacion: no corrió` · `corte: sin corte` · `error: sin error` · `pantalla: bienvenida`. O sea la vuelta llegó, el viaje estaba vigente, `execute()` pidió el SEGUNDO salto (la firma del PoP de KYC) y no hubo excepción, y a los 12,8 segundos la persona seguía en la bienvenida: la marca nunca pasó a `pop-kyc`. LA CAUSA: los DOS callbacks de este hook corren DENTRO DEL EFECTO DE MONTAJE (`alConectar` lo llaman `:4041` y `:4337`; a `alReanudar` lo llama `:4088`, y ese último con `yaInteractuo === false` GARANTIZADO por el gate de `:4080`), o sea que esa asignación era una navegación programática a una app externa FUERA DE TODO GESTO de la persona, y los navegadores móviles la descartan sin error y sin rastro. ⛔ NO SE ARREGLA CON UN `setTimeout` NI CON UN `requestAnimationFrame`: no es un problema de momento, es que no hay activación del usuario que consumir. El salto pasa a ser un `<a href>` (`:757`) que la persona TOCA, que es el patrón que este repo ya tiene desplegado y probado para el caso equivalente (el universal link de la billetera renderizado como `<a href>` en `:1379`, «es el que la persona toca a mano»). ⚠️ LA HIPÓTESIS RIVAL —que `r.irA` llegara vacío o mal formado— SE DESCARTÓ MIDIÉNDOLA, no razonándola: `T-075-GESTO-IRA` (`../infrastructure/solana/preparacion-por-enlace.test.ts`) corre el cableado de `../composition/container.ts:185` contra el disco real y el `irA` sale `https://phantom.app/ul/v1/signMessage?…` con `redirect_link` a nuestro origen. Y el `anotarHito` de arriba publica ahora la FORMA de ese destino en el bloque de diagnóstico, para poder volver a separar las dos causas EN EL TELÉFONO si algo cambia río arriba. ⛔ LOS OTROS `window.location.href` DE ESTE ARCHIVO NO SE TOCAN Y NO ES OLVIDO: `:162`, `:384`, `:460`, `:521` y `:626` corren todos adentro de un `onClick`/`onCrear`/`onAdoptar`, o sea DENTRO de un gesto, que es donde el navegador sí la honra — y `:333` sólo alcanza su rama `hay-que-salir` por el botón de `:964`, porque el otro llamador (`onConnect` desde esta misma línea) le pasa un `rc` que ya salió `listo`
  ...
  const guard = useCallback(async (fn: () => Promise<void>) => {
    ...
    try {
      await fn();
    } catch (e) { anotarHito("error", e instanceof Error ? (shortErrorCode(e.message) ?? "(mensaje vacío)") : "lo que se tiró no es un Error"); // HU-075/diagnóstico it2 — EN ESTA MISMA LÍNEA (Δ0, ver `:44`). ⛔ NO ES EXPOSICIÓN NUEVA: es EL MISMO `shortErrorCode(e.message)` que la línea de abajo ya le pasa al banner de `:1211`. Lo que cambia es DÓNDE se lee: ese banner se pinta DEBAJO del contenido del paso, así que en la bienvenida —que es la pantalla del reporte de campo— queda fuera de la captura y de la primera pantalla del teléfono. Acá sube al bloque, que está arriba de todo
      setError(
        e instanceof Error
  ...
  const onConnect = (vuelta?: { remittanceId: string; rc: Awaited<ReturnType<Container["connectWallet"]["execute"]>> }) =>
    guard(async () => { const via = vuelta === undefined ? "botón" : "vuelta"; anotarHito("continuacion", `${via}: corriendo`); // HU-075/diagnóstico it2 — EN ESTA MISMA LÍNEA (Δ0, ver `:44`). 🔴 EL ORIGEN SE FIJA ACÁ Y NO SE RECALCULA EN CADA SALIDA porque esta función tiene DOS llamadores con significados opuestos: el botón «Conectar wallet» de `:964` y la vuelta por enlace de `:286`. Sin la palabra, una captura tomada después de que la persona toque el botón se leería como el desenlace de la vuelta. ⛔ Y «corriendo» no es un adorno: es lo que queda escrito si un `await` de abajo TIRA — el código lo nombra el hito `error` de `:305`, y «corriendo» junto a un error es la firma de un corte por excepción
      const idRemesa = vuelta?.remittanceId ?? rem?.id; if (idRemesa === undefined) { anotarHito("continuacion", `${via}: cortó, sin id de remesa`); return; } // HU-075/reanudar — EN ESTA MISMA LINEA, reemplazando al `if (!rem) return;` que habia (Δ0: `:330` recibe 75 citas ancladas y este archivo 149 entrantes). 🔴 POR QUE EL ID ENTRA POR PARAMETRO Y NO SE LEE DE `rem`: al volver de la billetera por enlace la pagina SE REMONTA, `rem` es estado de React y arranca en `null`, asi que la unica forma de que ESTA funcion —la que ya sabe que hacer despues de conectar— corra para esa vuelta es que le pasen el id. El borrador no se perdio: la remesa la escribio `createRemittance` en el repo (localStorage) y el `lockQuote` de abajo la repone. ⛔ Y NO SE ESCRIBIO UNA SEGUNDA FUNCION «continuar despues de conectar»: la que hay es esta, y la vuelta por enlace la llama con el resultado del connect que YA tiene.
      const rc = vuelta?.rc ?? (await c.connectWallet.execute()); if (rc.estado === "hay-que-salir") { anotarHito("continuacion", `${via}: salió a la billetera`); window.location.href = rc.irA; return; } const { // WKH-359/AC-3 — LA NAVEGACIÓN, EN ESTA LÍNEA Y SIN AGREGAR NINGUNA (Δ0: `:330` recibe 75 citas ancladas y `:507`/`:521`, que están más abajo, 55 y 52). Igual que las otras dos suspensiones de esta pantalla, acá SÓLO se navega: la URL viene armada por el protocolo de la billetera y no se parsea, no se reescribe, no se le agrega nada
        address: addr,
        rememberedKyc,
        serverVerdict,
        kycProof: proof,
      ...
      const locked = await c.lockQuote.execute({ remittanceId: idRemesa });
      ...
      if (
        ...
        if (res.kind === "done") {
          setRem(res.snapshot);
          setStep("confirm"); anotarHito("continuacion", `${via}: navegó a confirm`);
        } else {
          setStep("review"); anotarHito("continuacion", `${via}: navegó a review (el atajo KYC-once no cerró)`);
        }
      } else {
        setStep("review"); anotarHito("continuacion", `${via}: navegó a review`);
      } if (vuelta !== undefined) void mirarLaCuentaDeNonce(addr).catch(() => {}); // HU-075/nonce — LA PREGUNTA POR LA CUENTA DE NONCE, ADELANTADA A UN MOMENTO ALCANZABLE POR ENLACE, Y EN ESTA MISMA LÍNEA FÍSICA (Δ0). 🔴 POR QUÉ ACÁ Y NO DONDE ESTABA. El otro sitio que la pregunta vive en la rama `conectado` del productor de montaje, DESPUÉS de un `if (r.estado === "hay-que-salir") return;`, y por el camino por enlace ese estado es SIEMPRE `hay-que-salir` (hay que saltar a la billetera a firmar el PoP), así que la pregunta NO CORRE NUNCA justo en el único recorrido que necesita la cuenta. ⛔ Ese `return` NO SE TOCA y su motivo sigue vigente: evita una lectura de red que nadie va a leer porque el navegador se está yendo a la billetera. La pregunta se mueve, el `return` se queda. 🔴 POR QUÉ ESTE MOMENTO SÍ ES SEGURO: acá la vuelta del `pop-kyc` YA ATERRIZÓ y la persona se QUEDA en esta pantalla (`review` o `confirm`), o sea que no hay ninguna navegación en curso que se lleve puesta la respuesta. Medido en el teléfono del founder el 2026-08-31: `continuacion: vuelta: navegó a review`. ⛔ EL GATE ES `vuelta !== undefined` Y NO OTRA COSA: ese parámetro sólo lo pasa la pata del enlace, así que el botón «Conectar wallet» del navegador de siempre NO gana una lectura de cadena que no necesita. ⛔ VA CON `void` Y NO CON `await`: la pantalla tiene que quedar usable ya, y la respuesta enciende la tarjeta cuando llegue. El `.catch` vacío es deliberado y NO tapa un diagnóstico: los tres desenlaces reales de preguntarle a la cadena vuelven como VALOR (incluido "no llegamos a preguntar"), así que lo único que cae acá es una explosión de configuración, y con eso lo correcto es no romper la continuación del connect. ⚠️ Y NO REEMPLAZA AL CINTURÓN: el corte fail-closed por cuenta ausente sigue entero. Esto sólo hace que la persona se entere ANTES de gastar una orden de payout.
    });

  // WKH-187/AC-2: la CTA "Continuar" del review lleva al KYC pero NO lo auto-inicia (navegación pura).
  const onContinue = () => setStep("verify"); const mirarLaCuentaDeNonce = async (direccion: string) => { setEstadoNonce(await c.recorridoPorEnlace.estadoDeLaCuentaDeNonce(direccion)); }; const onVolverAMirarLaCuentaDeNonce = () => guard(async () => { const dir = await c.connectedWallet.getConnectedAddress(); if (dir !== null) await mirarLaCuentaDeNonce(dir); }); const onCrearCuentaDeNonce = () => guard(async () => { const idParaElNonce = rem?.id ?? c.recorridoPorEnlace.remesaEnCurso(); if (idParaElNonce === null) { setError({ message: COPY_NONCE_SIN_ENVIO_EN_CURSO }); return; } yaInteractuoRef.current = true; const dir = await c.connectedWallet.getConnectedAddress(); if (dir === null) { setError({ message: COPY_NONCE_SIN_BILLETERA_CONECTADA }); return; } window.location.href = (await c.recorridoPorEnlace.crearCuentaDeNonce({ direccion: dir, remittanceId: idParaElNonce })).irA; }); // WKH-358/AC-5 — LOS TRES EN ESTA LÍNEA (Δ0 de citas, `:44`). ⚠️ `onCrearCuentaDeNonce` marca `yaInteractuoRef` porque ES una interacción de la persona: sin eso, un resume que resuelva mientras ella está en la app de la billetera navegaría por encima al volver. ⛔ La dirección sale de `getConnectedAddress()` y NO de `address`: en el camino por enlace el estado de React se pierde en el salto y el puerto lee el disco
  ...
  const onConfirm = () =>
    guard(async () => {
      ...
      const res = await c.confirmAndSend.execute({ remittanceId: rem.id, hrefDeLaVuelta: window.location.href }); // WKH-356: TRES desenlaces. 🔴 WKH-373 — EL HREF, EN ESTA MISMA LÍNEA (Δ0). Acá la barra ya está limpia (el paso 2 del productor de montaje corrió al montar), así que este valor es el mismo que `authorizePrincipal` leía en vivo: ⛔ NO hay cambio de comportamiento en el camino del botón. Lo que cambia es que el dato ENTRA por parámetro, que es lo que hace que el otro llamador —el que sí volvía de un salto— pueda pasar el href SUCIO.
  ...
  const adoptarCuentaConectada = () =>
    guard(async () => {
      ...
      setAddress(r.address); if (r.estado === "hay-que-salir") { window.location.href = r.irA; return; } // WKH-359/AC-3 — EN ESTA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=165]] citas ancladas, marcador verificado). La dirección se aplica IGUAL antes de salir, porque el `connect()` ya corrió y ya la conocemos: perderla obligaría a re-conectar al volver del salto
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col pb-segura-b pl-segura-l pr-segura-r pt-segura-t">
      ...
      {saltoPendiente !== null ? (<Aviso tono="atencion" className="mb-holgado text-body"><span className="block font-semibold">Falta una firma para seguir</span><span className="mt-ajustado block">Tu billetera quedó conectada. Para continuar necesitamos una firma más, y esa la tenés que dar vos desde tu billetera. Conectar y firmar son dos permisos distintos: conectar nos da tu dirección, y cada firma la autorizás vos por separado. Lo que cargaste sigue guardado.</span><a href={saltoPendiente} rel="noreferrer" className="mt-normal inline-flex min-h-[52px] w-full items-center justify-center gap-ajustado rounded-caja bg-cochineal px-5 text-body font-semibold text-white shadow-lift"><ExternalLink className="size-icono-sm shrink-0" /> Ir a firmar a mi billetera</a></Aviso>) : null}{/* 🔴 HU-075/gesto · EL SEGUNDO SALTO, COMO ALGO QUE LA PERSONA TOCA. Va EN ESTA LÍNEA (Δ0 de citas, `:44`) y ACÁ ARRIBA por el mismo motivo que el aviso del KYC que le sigue: a 375x667 las pantallas miden más que el viewport, así que algo puesto al final nace debajo del pliegue — y esto es LO ÚNICO que la persona puede hacer en ese momento. 🔴 ES UN `<a href>` Y NO UN `<button onClick>`, y la diferencia es el arreglo: el defecto era navegar desde un efecto de montaje, fuera de toda activación del usuario (ver el razonamiento entero en `:286`). Un `onClick` que asigne `location.href` también corre dentro de un gesto y también funcionaría, pero el `<a>` es el patrón que este repo YA tiene desplegado y probado para el caso equivalente (el universal link de la billetera renderizado como `<a href>` en `:1379`) y no le pone una capa de JS en el medio a la única salida que le queda a la persona. ⛔ EL `href` VA TAL CUAL: no se parsea, no se reescribe y no se le agrega ningún parámetro, que es la misma regla que `:521` ya declara para la navegación del depósito. ⛔ EL COPY NO DICE QUE ALGO FALLÓ, y no falló: la vuelta llegó bien, el viaje está vigente y la billetera quedó conectada; lo único que falta es una firma más. Tampoco dice «empezá de nuevo», que es el pecado que esta HU persigue desde el primer día. Sin em dashes (CD-16). Lo mide `T-075-GESTO-3`. ⚠️ `min-h-[52px]` y no `h-`: en una pantalla angosta este texto envuelve, y con alto fijo quedaría recortado — es el criterio que `touch-targets.test.tsx` ya declara para los controles que no son `<Button>`. ⛔ Y NO SE LIMPIA A MANO: tocar el enlace navega de verdad a la billetera, y al volver el árbol se remonta con `saltoPendiente` en `null` otra vez. 🔴 HU-075/aterrizaje — LA FRASE DE LOS DOS PERMISOS, EN ESTA MISMA LÍNEA (Δ0). La pregunta del founder fue textual: «por qué me pide dos firmas cuando conecto wallet, es raro». Son dos POR DISEÑO —conectar entrega la dirección, y el PoP prueba la posesión firmando un desafío del servidor (`iniciarPop`, `../infrastructure/solana/deeplink/pop-por-enlace.ts`)— y hasta acá la pantalla no lo decía en ninguna parte, así que la respuesta va en el copy y no en una explicación aparte. ⛔ LA FRASE ES GENÉRICA A PROPÓSITO Y NO NOMBRA AL PoP: este MISMO aviso se muestra para las firmas del DEPÓSITO (`firmar-tx` y `firmar-patrocinio`, que llegan por `alReanudar`), y ahí «esta firma prueba que la billetera es tuya» sería FALSO. Lo que vale para los tres casos es que conectar y firmar son permisos distintos y que cada firma se autoriza por separado. ⛔ Sin em dashes en el copy, sin decir que algo falló y sin prometer nada que no se haya medido (CD-16). Lo mide `T-075-GESTO-3` por lo que NO dice y `T-075-COPY-2FIRMAS` por lo que SÍ dice */}{avisoKyc !== null ? (<Aviso tono={avisoKyc.destino === "verify" ? "atencion" : "bueno"} className="mb-holgado text-body"><span className="block font-semibold">{avisoKyc.destino === "verify" ? COPY_RESUME_NO_PODEMOS_SEGUIR_AVISO : "Tu verificación quedó lista"}</span><span className="mt-ajustado block">No te sacamos de esta pantalla porque la estabas usando vos. El envío que dejaste a medias sigue guardado.</span><Button variant="outline" className="mt-normal" onClick={() => { const a = avisoKyc; setAvisoKyc(null); setRem(a.snapshot); if (a.error !== undefined) setError({ message: a.error }); setStep(a.destino); }}>{avisoKyc.destino === "verify" ? "Reintentar la verificación" : "Seguir con ese envío"}</Button></Aviso>) : null}{/* 🔴 WKH-063 (fix-pack 2, con SUS DOS CAMINOS ABIERTOS CERRADOS EN EL 3) · EL AVISO QUE REEMPLAZA AL PISÓN, y es NO MODAL a propósito: no tapa nada, no roba el foco y no navega. Se prende cuando el resume terminó y `yaInteractuoRef` (`:175`) está en `true`, o sea cuando antes había una navegación no pedida. 🔴 SON DOS VARIANTES Y NO UNA (fix-pack 3), porque el resume tiene dos desenlaces terminales y el `failed` también pisaba: con `destino === "verify"` el título NO puede decir que la verificación quedó lista, y el `tono` tampoco puede ser `bueno` (el verde de esta app es el del dinero que llega). La segunda frase es la MISMA en las dos ramas a propósito, y por eso dice "la estabas usando" y no "la habías elegido": el fix-pack 3 sumó el embudo, donde la persona no eligió un destino con la barra sino que entró a tipear, y "elegido" habría sido falso en ese camino. ⛔ EL BOTÓN NO ES DECORACIÓN, y desde el fix-pack 3 es más que un `setStep`: APLICA EL SNAPSHOT (`setRem`) que `aterrizar` (`:208`) no aplicó. Es el único camino de vuelta medido — la barra sólo ofrece los tres destinos, "Empezar un envío" crea una remesa NUEVA, y "Mis envíos" tampoco sirve para una remesa sin depósito autorizado, que se lista SIN control de fila (`openable`, `:3412`). ⚠️ VA ACÁ Y NO ABAJO DEL BLOQUE DE ERROR: a 375x667 las tres pantallas de destino miden más que el viewport (medido en el encabezado de `recuperar-composicion.test.tsx`), así que un aviso puesto al lado de la barra nace debajo del pliegue. ⛔ Δ0 DE LÍNEAS: entra en la línea en blanco que había acá, por las citas por número de este archivo (`:44`). */}{estadoNonce !== null ? (<TarjetaDeCuentaDeNonce estado={estadoNonce} ocupado={busy} onCrear={onCrearCuentaDeNonce} onVolverAMirar={onVolverAMirarLaCuentaDeNonce} onSeguirSinCrearla={() => setEstadoNonce(null)} />) : null}{/* WKH-358/AC-5 — EN ESTA LÍNEA (Δ0 de citas, `:44`) y ACÁ ARRIBA, junto al aviso del KYC, por el mismo motivo que ése: a 375x667 las pantallas miden más que el viewport, así que algo puesto al final nace debajo del pliegue. El componente vive al FINAL del archivo, con todo su razonamiento. ⛔ «Seguir sin crearla» sólo apaga la tarjeta y NO limpia nada: el corte del adaptador por cuenta de nonce ausente sigue siendo el cinturón y trae a la persona de vuelta a esta misma oferta */}{step === "bienvenida" && disponibilidadWallet === "none" && !vueltaSinResolver && avisoKyc === null ? (/* 🔴 WKH-373 — SE LE SACARON DOS CONDICIONES AL GATE, `saltoPendiente === null` y `estadoNonce === null`, y son EXACTAMENTE los dos estados en los que el camino por enlace NO está cerrando: `saltoPendiente` es «la vuelta pidió OTRO salto» (el bucle de «me sigue pidiendo muchas firmas») y `estadoNonce` es la tarjeta que enciende `deeplink_nonce_ausente`. La puerta al camino que el founder reporta que SÍ cierra —el navegador interno de la billetera— se cerraba justo cuando hacía falta. Lo miden `T-373-5` y `T-373-5c` (`./flow-reanudacion.test.tsx`), y `T-373-5b` es el control: con el corte en pantalla la salida ya estaba, también antes. ⚠️ `!vueltaSinResolver` ⛔ NO SE TOCÓ, y hay que decir por qué porque el diagnóstico de esta HU lo señalaba a él: la afirmación «esa bandera sólo se apaga cuando la vuelta resuelve BIEN» es FALSA y está medida — se apaga en `.finally(alResolverseLaVuelta)` (`:4093`), que corre en los seis desenlaces y en los dos `catch`. Mientras la vuelta se resuelve la pantalla muestra un spinner, y ofrecer una salida ahí sería ruido. */<Aviso className="mb-holgado space-y-normal text-body"><div className="flex items-center gap-ajustado"><Smartphone className="size-icono-sm text-cochineal" /><span className="font-semibold">¿Estás en un celular con Phantom?</span></div><span className="block">Abrí Chaski adentro de Phantom y no vas a tener que saltar a otra app en cada firma.</span><a href={urlDeSalidaAlNavegadorDeLaBilletera({ href: typeof window !== "undefined" ? window.location.href : "", origin: typeof window !== "undefined" ? window.location.origin : "", hayBorrador: rem !== null })} rel="noreferrer" className="mt-normal inline-flex min-h-[52px] w-full items-center justify-center gap-ajustado rounded-caja bg-cochineal px-5 text-body font-semibold text-white shadow-lift"><ExternalLink className="size-icono-sm shrink-0" /> Abrir Chaski en Phantom</a><Muted escala="label">No tengo Phantom · <a href={URL_INSTALAR_PHANTOM} rel="noreferrer" className="underline">Instalarla y crear mi billetera</a></Muted><Muted escala="label">Si estás en una computadora, instalá la extensión de Phantom o Solflare y recargá la página.</Muted></Aviso>) : null}{/* 🔴 WKH-372/W1 · I-2(a) — LA OFERTA DE SALTAR AL NAVEGADOR DE LA BILLETERA, EN ESTA MISMA LINEA (Δ0 de citas, `:44`) y ACA ARRIBA por el mismo motivo que sus tres vecinos: a 375x667 la pantalla mide mas que el viewport y algo puesto al final nace debajo del pliegue. 🔴 ES UN `<a href>` Y NO UNA NAVEGACION PROGRAMATICA, y esa es la unica forma que funciona: la doc de Phantom dice que estos enlaces los tiene que tocar una persona, y el founder lo midio en su telefono (el razonamiento entero esta en `:286`) — fuera de un gesto, el navegador movil descarta la navegacion SIN ERROR Y SIN RASTRO. ⛔ No se "arregla" con un `useEffect`, un `setTimeout` ni un `requestAnimationFrame`. NINGUNA DE LAS SEIS CONDICIONES ES DECORATIVA: `bienvenida` porque en `send` hay datos a medio cargar y ofrecer el salto ahi es ofrecer perderlos; `"none"` porque con `"injected"` la persona YA esta adentro y con `"unknown"` todavia no se sabe, y afirmar antes de saber es lo que `T-CABLE-4` prohibe; `saltoPendiente === null` porque si hay un salto pendiente ese enlace es LO UNICO que la persona puede hacer y dos enlaces compitiendo ahi es el defecto que la HU 075 cerro; `!vueltaSinResolver` por el mismo motivo que la tercera condicion de `NoWalletHere` (`:1351`): mientras no se sabe, no se afirma; `avisoKyc === null` porque ese aviso lleva un envio a medias con su snapshot y saltar lo pierde; `estadoNonce === null` porque la tarjeta del nonce es del camino de respaldo y tiene sus propias salidas. ⛔ EL TITULO ES UNA PREGUNTA Y NO UNA AFIRMACION: el detector no puede decir si la persona esta en un celular, y preguntar es lo unico honesto — misma disciplina que `NoWalletHere` ya aplica en `:1368-1369`. Y DICE "no vas a tener que saltar a otra app en cada firma" Y NO "cero saltos": lo primero es verificable (adentro del navegador de la billetera el camino por enlace no se enciende, lo mide `recorrido-en-el-navegador-de-la-billetera.test.tsx`), lo segundo omitiria el redirect del verificador de identidad de `:460`. 🔴 SON DOS ENLACES Y NO UNO: circula que un universal link `browse` cae solo en la pagina de descarga cuando la app no esta instalada, y ESO NO ESTA VERIFICADO — la doc de Phantom no lo dice. Con dos, el recorrido de instalacion no depende de esa incognita. ⛔ La nota de la computadora se repite aca y en `:1386` a proposito: son dos pantallas distintas y la persona ve una sola. Lo miden `T-372-W1-1`, `T-372-W1-2` y `T-372-W1-6` */}{step === "bienvenida" && aterrizaje.vinoConMarca && rem === null && aterrizaje.borradorEnElDisco === "sin-borrador" ? (<Aviso tono="atencion" className="mb-holgado text-body"><span className="block font-semibold">Acá no están los datos que cargaste antes</span><span className="mt-ajustado block">Cargá el monto y los datos de tu familiar otra vez. Es una sola pantalla.</span></Aviso>) : null}{/* 🔴 WKH-372/W1 · I-2(b) — EL AVISO DE ATERRIZAJE, EN ESTA MISMA LINEA (Δ0). 🔴 AR/MNR-4 — LA PRIMERA CONDICION ES `step === "bienvenida"`, Y ES DEL FIX-PACK: la decidio el ORQUESTADOR en el gate del AR, y ⛔ CAMBIA EL CONTRATO I-2(b) DEL STORY FILE, que especificaba TRES condiciones y no cuatro. SIN ELLA este aviso no tenia gate de pantalla: con la marca puesta y el disco vacio quedaba pintado tambien en `send`, `history` y `recuperar`, o sea «Aca no estan los datos que cargaste antes» clavado arriba de una lista vacia en «Mis envios». Esta HU existe porque el founder dijo que la experiencia no se veia profesional, y eso es exactamente eso. LA OFERTA DE AL LADO YA LLEVA ESE MISMO GATE con su motivo escrito, asi que esto es CONSISTENCIA con el vecino y no diseño nuevo. Lo mide `T-372-W1-7c`, y quitar la condicion lo pone rojo. POR QUE SE MIDE EN RUNTIME EN VEZ DE AFIRMARSE: el navegador de la billetera es OTRA PARTICION DE ALMACENAMIENTO. No es "volver al mismo origen": es el mismo origen en otro navegador. Y esta pregunta es NUEVA, distinta de las cuatro que el camino por ENLACE declara sin verificar: aquellas son sobre un salto que SALE Y VUELVE al mismo navegador; esta es sobre `browse`, que no vuelve: ATERRIZA EN OTRO. Confundirlas seria heredar una respuesta que no aplica. ⇒ EL CODIGO NO AFIRMA SI EL BORRADOR SOBREVIVE: LO PREGUNTA CUANDO ATERRIZA, y hay CUATRO desenlaces observables y no dos — (1) marca y SIN borrador en el disco: el aviso aparece; (2) marca y CON borrador: no aparece, porque no hubo nada que contar; (3) SIN marca: no aparece, porque a un visitante nuevo dentro del navegador de la billetera no se le puede hablar de datos que nunca cargo. El caso (3) es el que un `has()` perderia, y lo mide `T-372-W1-7`; (4) CR/`BLQ-BAJO-1` — marca y EL DISCO NO SE DEJA LEER: tampoco aparece, y esa es la mitad que faltaba. La condicion pide `borradorEnElDisco === "sin-borrador"` y ya no `!hayBorrador`, o sea que exige haber PREGUNTADO: con un `getItem` que tira, decirle a la persona que sus datos no estan seria publicar el resultado de una pregunta que no se pudo hacer. El hito lo dice aparte (`con-marca-disco-ilegible`) en vez de colapsarlo. Lo mide `T-372-W1-7f`. 🔴 ESTE AVISO ES EL INSTRUMENTO DE CAMPO, Y ACA VA QUE CONTESTA Y QUE NO. Si aparece: se aterrizo con la marca de una salida y el disco de ESTE navegador no tenia borrador ⇒ el almacenamiento no cruzo. Si NO aparece habiendo marca, hay que mirar el hito para saber cual de las otras tres es: cruzo, el disco no se dejo leer, o no hubo salida. ⚠️ CR/`BLQ-MEDIO-1` — LA FRASE QUE ESTABA ACA («si no aparece y el borrador esta, cruzo») ERA FALSA ANTE UNA RECARGA, y se corrigio por las dos puntas: la de arriba es la que se sostiene, y ademas `:146` CONSUME la marca de la barra apenas la lee, asi que una recarga ya no reencuentra un `?wb=1` que sobrevivio a una remesa cargada a mano. Nadie necesita `?diag=1` para leer el desenlace (1), que es el que la ola vino a cazar. ⛔ EL COPY NO EXPLICA LA CAUSA: "el navegador de Phantom guarda todo aparte" seria una afirmacion causal que nadie midio. Lo unico medido en ese instante es que el borrador no esta aca, y eso es lo unico que la frase dice. Tampoco dice "empeza de nuevo" ni "se perdio", que es el pecado que esta misma linea persigue por escrito mas arriba. ⚠️ NI EL DISCO NI LA MARCA DE LA URL SE LEEN ACA: los dos salen del estado `aterrizaje` de `:146`, que se toma UNA sola vez al montar, y de ahi — ⛔ esa cita va SIN ancla a proposito: este archivo lleva marcadores `[[CENSO ... entrantes]]` que viven en SIETE archivos, y uno de ellos (`flow-vm.ts`) esta fuera del alcance de esta ola, asi que una cita anclada nueva obligaria a editarlo — y de ahi lo toman los dos consumidores. Dos lecturas del mismo disco serian dos sitios que se corrigen por separado. 🔴 AR/MNR-3 — LA MARCA SE CONSUME, NO SE RECALCULA, y tambien es del fix-pack: hasta el AR esta condicion volvia a llamar a `vinoDeUnaSalidaConBorrador(window.location.href)` en CADA render mientras `:146` ya tenia esa misma respuesta capturada. Las dos mitades NO divergen hoy (verificado: `hrefSinRastroDeVuelta` no borra `wb`), asi que el arreglo es contra el futuro: el dia que alguien agregue `wb` a los parametros que se limpian, este aviso y el hito que lee el bloque de diagnostico —que salen de la MISMA foto— contestarian cosas distintas y nada lo cazaria. El docblock de `:146` ya declaraba ese principio para el disco; ahora vale para las dos mitades de la foto, que es lo unico que lo hace un principio y no una costumbre. ⚠️ Y SE FUE EL guard de `window` que esta condicion tenia adelante: no hacia falta ni antes, porque `aterrizaje` ya devuelve la foto en falso cuando no hay `window`. ⛔ Y NO entra al modulo puro de la salida a proposito: ese modulo se define sin `localStorage`, y quien sabe si hay borrador es ESTA pantalla */}
      ...
      <MotionConfig reducedMotion="user"><AnimatePresence mode="wait">
        ...
        >{/* WKH-063 (2º pase) · `flex flex-col` ENTRA EN LA LÍNEA DE ARRIBA Y NO EN UNA NUEVA, y este comentario cuelga del `>` por lo mismo: este archivo recibe muchas citas por número y una línea de más corre todas las que apuntan más abajo (el número y su definición están en `:44`, medidos UNA vez; acá decía "74", copiado de allá cuando allá ya era falso, que es el antipatrón que el auto-blindaje de esta HU prohíbe). 🔴 POR QUÉ HACÍA FALTA, medido sobre el build de producción con Chrome a 412x915: la bienvenida tenía 381px de vacío entre el CTA y la barra (42% del viewport), y para centrar su bloque hacía falta una altura contra la que centrarse. `min-h-full`/`h-full` en el hijo NO SIRVEN, y está MEDIDO y no razonado: con este `<div>` en `flex-1` a secas, el hijo con `min-height:100%` medía 644px dentro de un padre de 728px, o sea que la clase emitía una regla que resolvía a nada. Es el peor modo de falla posible (compila, se ve igual, no hace nada), el mismo que el docblock de `borderRadius` en `tailwind.config.ts` describe para un `rounded-xl2` olvidado. Con el padre en `flex flex-col`, un hijo que pide `flex-1` sí se estira a los 728px. ⛔ ESTO NO CENTRA NADA POR SÍ SOLO: el `justify-center` y el `flex-1` viven en el hijo (`Bienvenida`, `bienvenida.tsx:175`), así que las demás pantallas siguen ancladas arriba, que es lo que un formulario tiene que hacer. Los otros pasos pasan de bloque a ítem flex SIN cambiar de alto: un ítem flex sin `flex-grow` mide su contenido, y cada rama de acá abajo renderiza UN solo elemento. */}
          ...
          {step === "bienvenida" && saltoPendiente === null && !vueltaSinResolver && <Bienvenida onEmpezar={() => { yaInteractuoRef.current = true; setStep("send"); }} disabled={busy} />}{step === "bienvenida" && saltoPendiente === null && vueltaSinResolver ? (<Card className="mt-ajustado space-y-normal text-center"><Loader2 className="mx-auto mt-aire size-icono-lg animate-spin text-cochineal" /><h2 className="text-title font-bold">Volviendo de tu billetera…</h2><Muted className="mx-auto max-w-xs">Estamos leyendo lo que devolvió tu billetera. Un segundo.</Muted></Card>) : null}{/* 🔴 HU-075/aterrizaje — LAS DOS CONDICIONES NUEVAS Y LA CARD, EN ESTA MISMA LÍNEA (Δ0: este archivo recibe citas ancladas por número hasta `:4301`, ver `:44`). QUÉ ARREGLA, textual del founder: «cuando firmo la primera vez me bota al inicio y me dice que falta una firma … debería pedirme la firma sin necesidad de ir al inicio». MEDIDO antes de tocar nada (sonda del 2026-08-30, creada, corrida y borrada): con `dl=firmar-tx` en la barra y la reanudación devolviendo `hay-que-salir`, el `document.body` decía «Chaskitu plata a Perú, sin vueltas · Falta una firma para seguir … · Tu plata no pasa por Chaski …», o sea el aviso ENCIMA de la pantalla de entrada entera. 🔴 POR QUÉ NO SE «VUELVE AL PASO DONDE ESTABA» Y ESTO NO ES UNA VERSIÓN BARATA: el salto por enlace REMONTA el árbol, así que en ese instante no existe ningún paso al que volver, y `saltoPendiente` (`:163`) sólo lo escriben los dos callbacks del productor de montaje (`:286`), o sea que cuando vale algo el `step` es SIEMPRE el inicial. Reponer un paso exigiría leer la remesa, y el único lector de este container que puede hacerlo (`listHistory`) filtra por `ownerAddress`, que el borrador del connect todavía NO tiene (lo escribe `startKyc`) ⇒ para la mitad de los casos no hay paso que reponer. Lo que sí se puede afirmar es lo otro: la pantalla de entrada no es el lugar de esa persona, y se saca. 🔴 Y LA SEGUNDA CONDICIÓN (`vueltaSinResolver`, `:163`) CIERRA UNA CARRERA, no es simetría: ese productor espera hasta `TECHO_DISPONIBILIDAD_MS` (3 s, `../infrastructure/solana/disponibilidad-decidible.ts:49`) ANTES de resolver nada, y en esa ventana la pantalla de entrada ofrecía «Empezar un envío», que marca `yaInteractuoRef` (`:175`); un toque ahí manda la reanudación al `alAvisar` del pisón (`:4080`) en vez de al salto, y la persona se queda sin la firma que venía a dar. ⛔ NO ES UN SEGUNDO MECANISMO DE SALTO (la regla de `:286`): acá no se navega, no se guarda ningún `irA` y no se decide nada, sólo se elige qué se pinta. ⚠️ LA CARD NO AFIRMA QUE LA VUELTA HAYA SALIDO BIEN: dice que se está leyendo, que es lo único cierto en ese instante. Sin em dashes (CD-16) */}
 ...
 */
function NoWalletHere({ direccionConectada, vueltaSinResolver, hayBorrador }: { direccionConectada: string | null; vueltaSinResolver: boolean; hayBorrador: boolean }) { // WKH-372/W1 · I-3b — LA PROP NUEVA EN ESTA MISMA LINEA (Δ0): sin ella `tsc` se pone rojo, porque `:963` ya la pasa
  ...
  if (availability !== "none" || direccionConectada !== null || vueltaSinResolver) return null; // 🔴 HU-075/reanudar — LA TERCERA CONDICION, EN ESTA MISMA LINEA (Δ0). QUE ARREGLA, medido en el telefono del founder con el bloque de diagnostico: la disponibilidad se decide a los 1852 ms y la direccion llega DESPUES, cuando el consumidor de la vuelta termina de conectar. Entre esos dos instantes `availability` ya vale `"none"` y `direccionConectada` todavia vale `null` ⇒ las dos condiciones de al lado dejan pasar el aviso, y la persona que acaba de volver de firmar lee «no vemos ninguna wallet» y ve como accion principal un enlace que la saca a otro navegador. ⛔ EL ARREGLO ANTERIOR NO ESTABA MAL, LLEGABA TARDE: cubria el estado FINAL y no el TRANSITORIO. 🔑 Y ESTA CONDICION ⛔ NO ES UNA TERCERA FUENTE DE «HAY BILLETERA»: no afirma nada sobre la billetera, afirma que TODAVIA NO SE SABE —la barra traia una marca nuestra al montar y el consumidor no contesto—. Mientras no se sabe, no se afirma. ⚠️ SE APAGA SIEMPRE: el `.finally` de `:4092` corre en los seis desenlaces del consumidor, corte incluido, asi que el caso legitimo (escritorio o celular sin extension, sin conexion y SIN vuelta en curso) sigue viendo el aviso; eso lo mide el par de control de `aviso-sin-wallet-con-cuenta.test.tsx`. // 🔴 WKH-075/fix — LA SEGUNDA CONDICIÓN, EN ESTA MISMA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=165]] citas ancladas y mide [[CENSO src/presentation/flow.tsx lineas=4453]] líneas; una línea nueva las corre a todas). QUÉ ARREGLA, reportado por el founder en PRODUCCIÓN: «si sale la dirección arriba, pero rápidamente te manda al browser de Phantom». O sea que este aviso se le presentaba a alguien YA CONECTADO, y con `esElUnicoCamino` el enlace «Abrir Chaski en Phantom» es el BOTÓN PRIMARIO de la pantalla (`esElUnicoCamino`, `:1381`) ⇒ la app expulsaba a otro navegador justo a quien acababa de entrar. ⛔ EL AVISO ES MÁS VIEJO QUE LOS ENLACES PROFUNDOS: se escribió cuando «no hay billetera inyectada» significaba «acá no podés usar la app», y con `NEXT_PUBLIC_SOLANA_DEEPLINK_ENABLED=true` esa premisa es FALSA — `availability` sigue diciendo `"none"` CORRECTAMENTE (no hay extensión) y la persona igual está conectada. 🔑 POR QUÉ EL PREDICADO ES «HAY DIRECCIÓN» Y NO «HAY DIRECCIÓN O EL CAMINO POR ENLACE ESTÁ DISPONIBLE»: la disponibilidad del camino por enlace es `deeplinkEnabled()`, una env GLOBAL que hoy está prendida en producción, y este componente ya sólo renderiza con `availability === "none"` ⇒ esa segunda variante lo apagaría SIEMPRE en producción, incluido el escritorio sin extensión y sin conectar, que es justo el caso en que el aviso es correcto y útil. El «instante previo a conectar» que la variante suprimiría ES ese caso legítimo, no un falso positivo. ⛔ Y NO SE INVENTA UN TERCER CRITERIO DE «CONECTADO» NI UNA PERILLA NUEVA: `direccionConectada` es el estado (`address`, `:163`), el MISMO sumidero al que escriben los cuatro caminos de conexión —la vuelta por enlace (`setAddress`, `:286`), el `onConnect` (`setAddress`, `:339`), el `ensureAddress` (`setAddress`, `:408`) y el `adoptarCuentaConectada` (`setAddress`, `:626`)— y la MISMA fuente que pinta el chip de la dirección. ⛔ NO sirve `useConnectedWalletAddress()`: lee `solanaWalletBridge.getState().publicKey`, que lo empuja el efecto de (`setState`, `./solana/solana-providers.tsx:178`) desde el `publicKey` del adapter INYECTADO, así que por el camino del enlace es `null` — o sea que daría `null` exactamente en el caso que este arreglo cierra. ⚠️ Las cuatro anclas `setAddress` son de las DÉBILES que el candado declara (el símbolo aparece en varias líneas): prueban que la línea habla de `setAddress`, no que sea LA línea.
  ...
  return (
    ...
    <Aviso className="space-y-normal">
      ...
      <Muted escala="label">
        Si al llegar no ves lo que cargaste, cargalo otra vez. <a href={URL_INSTALAR_PHANTOM} rel="noreferrer" className="underline">Instalar Phantom y crear mi billetera</a>. Si estás en una computadora, instalá la extensión de Phantom o Solflare y recargá la página.
      </Muted>
    </Aviso>
  );
}
            ...
            : senderSolMissing
              ...
              : prepareRejected
                ...
                : principalInEscrow
                  ...
                  : copyDeEntregaFallida(rem)} {/* WKH-233 (fix-pack it2 · AR/BLQ-ALTO-1 + AR/BLQ-MED-1) — EN ESTA LÍNEA (Δ0). ⚠️ ACÁ HABÍA UN `humanError("payout_failed")` HARDCODEADO, y era el agujero: el `failureReason` REAL de la remesa no llegaba nunca a `humanError` salvo por las dos ramas que lo pasan a mano más arriba, así que los copys propios que WKH-333 y este mismo fix-pack escribieron para `payout_not_authorized` y `payout_authority_unavailable` eran INALCANZABLES desde la pantalla. Medido renderizando ESTA vista: 12 de los 14 enums de `app/api/payout/prepare/route.ts` leían "si tus USDC entraron al escrow, los sacás vos firmando" sobre cortes con `principalTx === null`. ⛔ Y EL REEMPLAZO NO ES UN TERNARIO MÁS NI UNA LISTA DE ENUMS —el patrón que ya falló dos veces—: (`copyDeEntregaFallida`, `./flow-vm.ts:1602`) decide por el ESTADO DEL DEPÓSITO, así que un enum nuevo entra al mecanismo sin que nadie lo declare. El candado que lo re-mide en cada `npm test` vive en `copy-de-prepare-en-pantalla.test.tsx` (T-PANT-2), y deriva el conjunto de enums leyendo la route */}
        </Muted>
...
export function LostEscrowRecovery({
  ...
  // (`esVentanaSinAbiertos`, `flow-vm.ts:1145`), en su docblock. ⚠️ Y desde el fix-pack de WKH-347 `escrow_index_absent` SÍ lo prende: es cierto (a ese código se llega con la ventana del servidor ya recorrida y sin ninguno abierto) y sin eso el cierre de ventana no se prendía para NINGUNA billetera existente. El motivo medido está en ese mismo docblock.
 ...
 * y un solo fetch a `/compose` (T-1.2, MEDIDO: `it.each`, `../../app/api/payout/prepare/route.test.ts:1407`). Por
 ...
 * `flow-vm.ts:1339`) —el reparto lo hace (`historyGroupFor`, `flow-vm.ts:1377`)— y las renderiza en el
 * orden de (`HISTORY_GROUP_ORDER`, `flow-vm.ts:1342`), que es severidad decreciente. No calcula ningún
 * desenlace: usa el mismo (`escrowOutcome`, `flow-vm.ts:1201`) que la tarjeta ya usaba, y la tarjeta se
 ...
 * docblock de (`HistoryGroup`, `flow-vm.ts:1339`) —no en el de `historyGroupFor`, que habla del `??`—;
...
const NO_WALLET_CON_MWA =
  "En este navegador, al tocar Conectar wallet puede abrirse la app de tu billetera para que autorices desde ahí. Si eso no pasa, usá el enlace de abajo y abrí Chaski adentro de Phantom."; /** 🔴 WKH-372/W3.5 · AC-3-6 — LAS CUATRO FRASES DE LA FIRMA DE IDENTIDAD, EN ESTA MISMA LÍNEA (Δ0). ⛔ SE EXPORTAN Y SE IMPORTAN, no se escriben dos veces: el `it` que las vigila (`T-372-W3-10`, por nombre, en `./wallet-availability.test.tsx`) IMPORTA esta constante. Si el texto viviera también en el test, el guard se estaría leyendo a sí mismo y NUNCA podría fallar. ⛔ SE MUESTRAN LAS CUATRO SIEMPRE, en `"injected"` y en `"none"`, SIN GATE. La 4ª estuvo a punto de gatearse a `"injected"` con el texto «se hace una sola vez para esta dirección», y se descartó porque el gate NO la vuelve verdadera: la sesión que evita la segunda firma vive SÓLO en memoria, y el recorrido de primera vez RECARGA la página en (`window.location.href = res.url;`, `:460`) ⇒ toda persona que se verifica en este envío pierde la sesión y firma de nuevo. Publicar «una sola vez» sería mentirle a cada remitente nuevo. La frase que quedó es verdadera en los DOS caminos y en las DOS ramas. ⚠️ CADA FRASE TIENE EL INPUT QUE LA PONDRÍA EN ROJO: la 1ª y la 4ª, `T-372-W3-10b` (por nombre, en `./sesion-borra-la-segunda-firma.test.tsx`), que reconstruye cada mensaje firmado con `buildSolanaPopMessage` y falla si la app manda a firmar otra cosa; la 2ª y la 3ª, ese mismo `it`, que además exige que el mensaje NO contenga el monto ni el destino del beneficiario. ⛔ NINGUNA dice que algo falló ni usa un pasado sobre haber revisado, ⛔ ninguna toca el asunto del gas nativo del remitente (el depósito NO es gasless para él), y ⛔ ninguna vuelve falso el `<h2>Tu plata no pasa por Chaski</h2>` de `./bienvenida.tsx:232`: la sesión NO es una clave, no firma transacciones y no autoriza movimiento de fondos. Sin em dashes. */ export const COPY_FIRMA_DE_IDENTIDAD = ["Te vamos a pedir una firma para confirmar que la billetera es tuya.", "Es gratis.", "No mueve tus USDC ni autoriza ningún pago.", "Si te la volvemos a pedir, es por lo mismo: confirmar que es tuya."] as const;
...
function useVueltaPorEnlace(i: {
  ...
  alConectar: (r: Awaited<ReturnType<Container["connectWallet"]["execute"]>>, remittanceId?: string) => void; alResolverseLaVuelta: () => void; // HU-075/reanudar: el 2do parametro EN ESTA MISMA LINEA (Δ0: `:3956`, `:3999`, `:4005`, `:4009`, `:4010`, `:4023`, `:4027`, `:4070`, `:4198`, `:4243` y `:4301` reciben citas por numero). Es el id de la remesa EN CURSO, y es lo unico que le falta a `onConnect` para retomar el recorrido despues de un remonte. ⛔ OPCIONAL porque la vuelta del permiso puede llegar SIN viaje (el gate de `:4010` no corrio todavia): sin id, `alConectar` hace lo de siempre y no retoma nada ⚠️ Y `alResolverseLaVuelta` VA EN ESTA MISMA LINEA Y DELANTE DEL `//`, no debajo: una linea propia aca corre las citas por numero de `:3999` para abajo, y pegarla DESPUES del comentario la dejaria comentada sin que `tsc` la cace hasta que algo la use (le paso dos veces a `:74`/`:77` en esta misma HU). Que declara: «el consumidor de la vuelta termino», pase lo que pase — lo llama el `.finally` de `:4092` en los SEIS desenlaces, corte incluido. ⛔ NO dice si conecto: dice que ya no hay nada pendiente que averiguar, que es lo unico que el aviso «no vemos ninguna wallet» necesita para volver a poder afirmar
  ...
  const { c, yaInteractuo, alConectar, alResolverseLaVuelta, alFallar, alReanudar, alAvisar, alSaberDelNonce } = i;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const marca = marcaDeVuelta(hrefAlMontar); anotarHito("href-al-montar", formaDelHref(hrefAlMontar)); // WKH-373: el hito, EN ESTA MISMA LÍNEA (Δ0). ⛔ `formaDelHref` NO imprime el href: informa la PRESENCIA de cada parámetro de respuesta, porque `data` es el sobre cifrado y la clave de la billetera viaja al lado.
      ...
      if (esVueltaPorEnlaceNuestra(hrefAlMontar)) { const d = await esperarDisponibilidadDecidible(); if (!vivo.valor) return; if (d.estado === "sin-decidir") { limpiarLaBarra(); alFallar(d.causa); return; } } const remId = c.recorridoPorEnlace.remesaEnCurso(); // WKH-075 — LA ESPERA, EN ESTA LÍNEA (Δ0) Y ANTES DE LAS DOS PUERTAS. 🔴 EL DEFECTO ERA DE MOMENTO, NO DE VALOR: este productor corre a `t ≈ 0` y ahí `getWalletAvailability()` todavía vale `"unknown"`; el gate de la rama de enlace es `!== "none"` (`../infrastructure/solana-wallet.ts:2240`), así que `"unknown"` cae del lado de `"injected"`, `connect()` termina en `openModal()`, el selector de la librería se abre, se auto-cierra a los 150 ms y la persona lee «Se cerró el selector de wallet sin conectar» **después de haber firmado**. MEDIDO (foto del 2026-08-29, ⛔ NO re-derivable desde esta suite): `t=50ms availability=unknown selector-en-el-DOM=true` · `t=1750ms availability=none`. El dato nunca faltó: llegaba tarde. ⚠️ EL INSTRUMENTO DE ESOS DOS NÚMEROS FUE LA SONDA 2 DE `doc/sdd/075-la-vuelta-.../sdd.md` §0.2 —árbol de providers real, adapters reales, reloj real—, un archivo temporal que se creó, se corrió y se BORRÓ (fix-pack · AR/MNR-3). 🔴 LA CARRERA ENTRA POR LAS DOS PUERTAS Y ESO ESTÁ MEDIDO (`dl=pop-kyc` ⇒ `:4009`, `dl=conectar` ⇒ `:4038`, las dos con `openModal.llamadas=1` — foto del 2026-08-29, instrumento: la SONDA 1 de `doc/sdd/075-la-vuelta-.../sdd.md` §0.1, que monta la pantalla SIN árbol de providers y por eso puede leer el contador del bridge; ⛔ ESE NÚMERO NO SE RE-DERIVA con el mismo aparato desde `vuelta-por-enlace-carrera.test.tsx`, que prohíbe por escrito espiar `registerOpenModal` porque el árbol real PISA el doble y un cero ahí significa «no medí» y no «no pasó»), así que la espera va en el ÚNICO punto por el que pasan las dos. Arreglar una sola mueve el síntoma, no lo saca. ⛔ Y NO SE ARREGLA CAMBIANDO EL GATE A `!== "injected"`: está prohibido por escrito en el propio adaptador y lo mata `T-065-GATE-3` —con esa forma, un escritorio con la extensión todavía sin montar entraría a la rama de enlace, que es el camino del video del Demo Day—. ⚠️ EL GATE SON TRES CONDICIONES Y CADA UNA HACE FALTA. (1) `marca !== null` es lo que hace el cambio INVISIBLE para todo lo demás: los ~100 `it` que montan esta pantalla sin marca no ejecutan una sola línea nueva. (2) 🔴 `MARCAS_DE_VUELTA.includes(marca)` ES UNA CORRECCIÓN DEL FIX-PACK 1 (CR/BLQ-1): acá decía SÓLO `marca !== null`, y `marcaDeVuelta` devuelve la marca CRUDA SIN VALIDAR, así que CUALQUIER `?dl=` ajeno entraba a la espera — MEDIDO con `?dl=una-marca-que-nadie-escribio` en la barra: la pantalla terminaba mostrando el copy del techo. Es exactamente el razonamiento que este mismo archivo ya tenía escrito 65 líneas más abajo (`:4070`) y que esta línea no aplicaba. Lo mide `T-075-1c`. (3) 🔴 `deeplinkEnabled()` ES LA OTRA CORRECCIÓN (AR/BLQ-5, AC-4): ⛔ NO es una tercera perilla —eso es lo que CD-3 prohíbe— sino LA MISMA env aplicada en un sitio más; sin ella, con la bandera apagada (que es EL repliegue del BUILD, y el escenario es apagarla con gente a mitad de viaje) una vuelta con `?dl=` bloqueaba el consumidor de montaje hasta la gracia y podía mostrar un copy que `b71e917` no tenía. Lo mide `T-075-4b`, y su gemelo `T-075-4c` cubre la señal de `:4070`, que cuelga de la misma bandera por el mismo motivo. ⚠️ Y ACÁ DECÍA que «el único archivo de presentación que siembra `dl=` escribe siempre `availability="none"`, con lo que la espera resuelve en el primer tick», Y ESO YA NO ES CIERTO: el fix-pack agregó a `flow-reanudacion.test.tsx` un arnés que deja la disponibilidad SIN DECIDIR a propósito, y es justamente lo que vuelve alcanzable la rama del techo desde un render. ⚠️ `if (!vivo.valor) return;` DESPUÉS DEL `await` es la convención de este mismo productor (`:4024`, `:4040`, `:4078`): la pantalla puede desmontarse durante la espera. ⛔ LA CAUSA LA DEVUELVE EL MÓDULO (`d.causa`) Y NO SE ESCRIBE COMO LITERAL ACÁ, ni siquiera dentro de un comentario, que es la regla que este archivo ya declara en `:4013`. ⛔ Y EL TECHO NO DEGRADA A `"none"`: sale por su causa propia, corta, y NO llama a `connectWallet.execute()` ni abre nada.
      ...
      if (res.estado === "conectado") {
        ...
        try {
          ...
          const r = await c.connectWallet.execute();
          if (!vivo.valor) return;
          alConectar(r, remId); if (r.estado === "hay-que-salir") return; // HU-075/reanudar — EL `remId`, EN ESTA MISMA LINEA (Δ0). Es lo que hace que `onConnect` pueda retomar el recorrido despues del remonte: sin el, esta rama repoblaba la conexion y dejaba a la persona en la pantalla de entrada con el formulario ya lleno. Aca `remId` es `string` (lo estrecho el gate de `:4010`). // WKH-359/AC-3 — EN ESTA LÍNEA (Δ0). `alConectar` ya navegó; sin este `return` el recorrido seguiría preguntándole a la CADENA por la cuenta de nonce mientras el navegador se va a la billetera, o sea una lectura de red que nadie va a leer
      ...
      if (marca !== "firmar-tx" && marca !== "firmar-patrocinio" && marca !== "pop-payout") { if (esVueltaPorEnlaceNuestra(hrefAlMontar) && vivo.valor) alFallar(DEEPLINK_MARCA_SIN_CONSUMIDOR); return; } // WKH-075 — LA SEÑAL, EN ESTA MISMA LÍNEA FÍSICA (Δ0: hay 4 citas ancladas de acá para abajo). Este `return` era MUDO: una marca CONOCIDA que llegaba hasta acá sin que ninguna rama la reclamara moría en silencio, y la persona no leía NADA después de haber firmado. ⛔ SE GATEA POR «MARCA CONOCIDA» Y NO POR `marca != null`: `marcaDeVuelta` devuelve la marca CRUDA SIN VALIDAR (`../infrastructure/solana/deeplink/conexion.ts:338`), así que una marca basura o de otro sistema NO dispara nada, que es lo que el AC pide. La lista es un VALOR y no una copia a mano (`MARCAS_DE_VUELTA`, `../infrastructure/solana/deeplink/sesion.ts:495`). ⚠️ ACÁ SEGUÍA «así que agregar una marca sin consumidor SE VE», que es la frase gemela de la que el fix-pack ya corrigió en esa misma línea de `sesion.ts` (AR+CR/BLQ-4), y hay que decir de qué se ve y de qué no: es FALSA sobre la SUITE —está medido: un 7º elemento en esa tupla, líneo-neutro, deja `biome lint`, `tsc --noEmit` y `vitest` los tres en verde— y es CIERTA sobre el RUNTIME, porque una marca conocida que ninguna rama reclame cae al fallthrough de ESTA misma línea y dispara `DEEPLINK_MARCA_SIN_CONSUMIDOR`, que la persona SÍ lee. Lo que se ve es la SEÑAL EN RUNTIME, no un candado de la suite: nada se pone rojo, y quien agregue una marca tiene que darle rama o aceptar esa señal. ⛔ `pop-payout`, `firmar-tx` y `firmar-patrocinio` SIGUEN DE LARGO y no disparan la señal: son las tres que este `if` deja pasar, y lo vigila `T-075-5b`. ⚠️ El `vivo.valor` es la misma convención de `:4020`: entre el último chequeo y acá hay `await`s. ⛔ Y no se vuelve a llamar `limpiarLaBarra()`: ya corrió en `:4023`, para TODA vuelta. // WKH-359/AC-7 — LA RAMA DEL PERMISO, PEGADA A LA LÍNEA QUE EXISTE (Δ0: hay 4 citas ancladas de acá para abajo y ésta es la única línea de este archivo que esta HU toca; `:286`, `:330`, `:507` y `:521` reciben 77, 75, 55 y 52 y NO se tocan). 🔴 LA REANUDACIÓN SIGUE SIENDO POR RESULTADOS Y NUNCA POR `viaje.paso` (AC-7): esto no pregunta en qué paso dice el viaje que está —el campo (`paso`, `../infrastructure/solana/deeplink/sesion.ts:154`) de `Viaje` queda RANCIO por construcción, dice qué se fue a pedir en el salto en curso y no qué se consiguió— sino que lee el ancla y sigue SÓLO si la firma volvió y VERIFICÓ. Cuando sigue, cae al MISMO bloque de abajo que la reanudación de siempre: el dueño, la remesa en `confirmed`, y `execute()`. ⛔ Y no se le vuelve a pedir ninguna firma ya dada: `execute()` encuentra la prueba anclada y `pedir()` la entrega sin saltar (lo mide `T-067-11`). ⚠️ El motor NO tocó nada al pasar por acá: `pop-payout` no es un `PasoDelViaje`, así que `completar()` de más arriba contestó `nada` sin consumir ni destruir el viaje del depósito (CD-11, `T-067-16`)
      ...
      try {
        const r = await c.confirmAndSend.execute({ remittanceId: remId, hrefDeLaVuelta: hrefAlMontar }); anotarHito("href-al-reanudar", formaDelHref(hrefAlMontar)); // 🔴 WKH-373 — EL ARREGLO, Y ES ESTA LÍNEA. `hrefAlMontar` se capturó en `:3996`, ANTES de que `limpiarLaBarra()` (`:4023`) le sacara a la barra el `dl`, el `nonce`, el `data` y la clave de cifrado. Hasta acá `authorizePrincipal` leía `globalThis.location.href` EN VIVO (`../infrastructure/solana-wallet.ts:1177`) ⇒ veía la URL YA limpiada, el lector de la vuelta del motor contestaba «no volvimos», el motor re-anclaba y devolvía OTRO salto: la MISMA firma pedida de nuevo en cada vuelta, y la persona sin salir de la bienvenida. ⛔ Y el hito va pegado acá y no en otro lado: es lo que hace VISIBLE en `?diag=1` qué href recibió el consumidor, o sea lo único que separa esta causa de las otras cuatro sin volver a probar en un teléfono.
        if (vivo.valor) alReanudar(r);
      } catch (e) {
        if (vivo.valor) { const causa = (e as Error).message; alFallar(causa); if (causa === DEEPLINK_NONCE_AUSENTE) alSaberDelNonce("falta"); } // HU-075/nonce — EL CORTE ENCIENDE SU PROPIA REPARACIÓN, EN ESTA MISMA LÍNEA FÍSICA (Δ0). 🔴 QUÉ ESTABA ROTO: `deeplink_nonce_ausente` tenía COPY y nada más. El copy le pide a la persona que cree la cuenta, y la tarjeta que la crea existe y está montada, pero su interruptor (`alSaberDelNonce`) NO lo tocaba ninguno de sus llamadores en este camino, así que la pantalla le pedía crear algo sin darle con qué. Callejón, medido en el teléfono del founder el 2026-08-31 con el bloque `?diag=1`: `marca al montar: pop-payout` · `pantalla: bienvenida` · `corte: deeplink_nonce_ausente`. ⛔ EL COPY NO SE TOCA NI SE REEMPLAZA: `alFallar(causa)` sigue primero y la persona tiene que seguir leyendo que NO se firmó nada. Lo que se agrega es la tarjeta, no otro texto. ⛔ LA CAUSA SE COMPARA CONTRA LA CONSTANTE IMPORTADA y no contra un literal escrito acá: el literal se desincroniza en silencio si el vocabulario cambia. ⛔ Y SÓLO ESTA CAUSA ENCIENDE LA TARJETA: cualquier otro corte (saldo, rechazo, viaje vencido) NO habla de la cuenta de nonce, y ofrecer crearla ahí sería mandar a pagar un alquiler que no arregla nada.
      }
    })().finally(alResolverseLaVuelta); // HU-075/reanudar — EN ESTA MISMA LINEA (Δ0). 🔴 POR QUE `.finally` DEL IIFE Y NO UN `alResolverseLaVuelta()` EN CADA SALIDA: este productor tiene SEIS desenlaces (corte, conectado, los tres del nonce, la reanudacion) mas tres retornos mudos y dos `catch`; poner el aviso en cada uno es exactamente la clase de arreglo que se olvida de una rama, y la rama olvidada deja el aviso «no vemos ninguna wallet» apagado PARA SIEMPRE en esa pestaña. ⛔ NO cambia el manejo de errores: `.finally` re-propaga, asi que la promesa flotante sigue rechazando igual que antes.
    return () => {
...
const AVISO_REANUDACION_POR_ENLACE =
  "Volviste de tu billetera y dejamos ese envío como estaba, porque estabas usando esta pantalla. Lo encontrás en Mis envíos para seguir desde donde quedó."; /** HU-075/nonce — LO QUE LEE LA PERSONA CUANDO EL BOTÓN «Crear la cuenta» NO PUEDE ARMAR NADA. ⛔ Las dos van EN ESTA MISMA LÍNEA FÍSICA (Δ0). 🔴 EXISTEN PORQUE EL BOTÓN ERA MUDO: sus dos guardas (`rem === null` y la dirección ausente) hacían `return` sin escribir nada, y tras la vuelta por enlace `rem` está SIEMPRE en `null`, así que el único camino que necesitaba el botón era exactamente el que no decía nada. ⛔ NINGUNA DE LAS DOS DICE QUE ALGO FALLÓ, porque no falló: en las dos se sale ANTES de armar la transacción, así que no se pidió ninguna firma y eso se dice explícito. ⛔ Sin em dashes (CD-16). */ export const COPY_NONCE_SIN_ENVIO_EN_CURSO = "No encontramos a qué envío corresponde esta cuenta, así que no armamos nada. No se pidió ninguna firma. Empezá el envío de nuevo y creá la cuenta desde ahí."; export const COPY_NONCE_SIN_BILLETERA_CONECTADA = "No hay ninguna billetera conectada en este navegador, así que no podemos armar la creación de la cuenta. No se pidió ninguna firma. Conectá tu billetera y probá otra vez.";
...
export function TarjetaDeCuentaDeNonce({
  ...
  return (
    <Aviso tono="atencion" className="mb-holgado text-body">
      ...
      {/* 🔴 ESTE PÁRRAFO YA SE EQUIVOCÓ DOS VECES, Y LA TERCERA VERSIÓN ES LA ÚNICA MEDIDA EN UN TELÉFONO. (1) Decía «LA SALIDA SECUNDARIA NO ES UN CALLEJÓN» apoyándose en que el corte del adaptador por cuenta ausente ((`deeplink_nonce_ausente`, `../infrastructure/solana-wallet.ts:818`)) traería de vuelta a esta misma oferta: el AR lo declaró FALSO (AR/BLQ-BAJO-1), porque volver a la oferta no era lo que pasaba. (2) Pasó entonces a decir que ese corte era INALCANZABLE por el camino por enlace, porque `prepare()` moría antes con `payout_pop_unavailable` (WKH-359) y la remesa terminaba en el desenlace de `prepareUnreachable`. ⛔ ESA SEGUNDA VERSIÓN TAMBIÉN QUEDÓ VIEJA, y no la voltea un razonamiento sino una medición: el 2026-08-31, en el teléfono del founder y con el bloque `?diag=1` abierto, la foto dice `marca al montar: pop-payout` · `viaje.remesa: f3ce04…1a96 · repo: confirmed` · `pantalla: bienvenida` · `corte: deeplink_nonce_ausente`. O sea que con los arreglos de HU-075 `prepare()` PASA y el corte SE ALCANZA. (3) Y desde HU-075/nonce ese corte además ENCIENDE esta tarjeta: el `catch` de la reanudación por enlace llama a `alSaberDelNonce("falta")` junto al copy, así que la persona sí vuelve a esta misma oferta, y ahora con el botón puesto y con qué tocar.
          ...
          ⚠️ Y NO SE PROMETE que el depósito por enlace cierre de punta a punta cuando la cuenta exista: NADIE de este equipo lo corrió entero en un teléfono. Lo que está medido es hasta acá —el corte se alcanza y trae de vuelta a esta oferta—; lo que la cuenta habilita es el paso del nonce, no el pago. */}
...
async function resolverVueltaDelPermiso(i: {
  ...
  hrefAlMontar: string; remId: string | null; // HU-075/reanudar: el 2do EN ESTA MISMA LINEA (Δ0). ⛔ `null` es alcanzable —el permiso es de una BILLETERA y se resuelve aunque el viaje ya no exista, que es justo por lo que esta rama corre ANTES del gate de `:4010`—, y con `null` no se retoma nada
  limpiarLaBarra: () => void;
  vivo: { readonly valor: boolean };
  yaInteractuo: { readonly current: boolean };
  alConectar: (r: Awaited<ReturnType<Container["connectWallet"]["execute"]>>, remittanceId?: string) => void;
  alFallar: (causaCruda: string) => void;
  alAvisar: (mensaje: string) => void;
}): Promise<"cortar" | "seguir"> {
  const { c, marca, hrefAlMontar, remId, limpiarLaBarra, vivo, yaInteractuo, alConectar, alFallar, alAvisar } = i;
  ...
  if (marca === "pop-kyc") {
    ...
    try {
      ...
      if (vivo.valor) alConectar(rk, remId ?? undefined); // HU-075/reanudar — el id EN ESTA MISMA LINEA (Δ0). 🔴 ESTE ES EL ATERRIZAJE REAL EN PRODUCCION Y NO EL DE `:4030`: desde la pantalla `connect` la persona salta con `dl=conectar`, vuelve, y `connectWallet.execute()` le pide LA FIRMA del PoP de KYC, o sea un SEGUNDO salto con `dl=pop-kyc`. La vuelta de ese segundo salto cae aca, y hasta esta HU se quedaba en la pantalla de entrada igual que la otra. ⛔ Y NO se vuelve a llamar `connectWallet.execute()` mas abajo: la prueba del PoP por enlace es de UN SOLO USO (`leerPruebaPop` borra el ancla antes de devolver), asi que una segunda llamada saldria `hay-que-salir` y mandaria a la persona a firmar OTRA VEZ. Por eso `onConnect` recibe el `rc` que ESTA linea ya tiene, en vez de conectar de nuevo.
    } catch (e) {
      if (vivo.valor) alFallar((e as Error).message);
    }
    return "cortar";
 ...
 * nadie había medido. Ahora el copy sale de (`copyDelFinDelResume`, `./flow-vm.ts:1747`), que resuelve
...
/**
 * HU-075/reanudar — ¿LA BARRA TRAE UNA VUELTA POR ENLACE QUE ESTE REPO ESCRIBIO?
 *
 * 🔴 POR QUE EXISTE, Y POR QUE ⛔ NO ES UNA FUENTE DE VERDAD NUEVA. Estas tres condiciones ya estaban
 * escritas DOS veces adentro de `useVueltaPorEnlace` —la espera de `:4005` y la señal del fallthrough
 * de `:4070`—, iguales y sin nada que las atara. Esta HU necesitaba una TERCERA lectura, la del aviso
 * «no vemos ninguna wallet», que ⛔ no puede afirmar nada mientras una vuelta se esta resolviendo. Una
 * tercera copia a mano es exactamente el defecto que esta HU vino a cerrar (dos fuentes de verdad de
 * «hay billetera» fue el bug del arreglo anterior) ⇒ el predicado se escribe UNA vez y los tres lo
 * leen. Los tres sitios quedan atados por construccion: cambiarlo aca los cambia a los tres.
 *
 * ⛔ QUE **NO** DICE: no dice «hay billetera», no dice «conecto» y no dice «la firma sirve». Dice que
 * la barra trae una marca del vocabulario de `MARCAS_DE_VUELTA` y que el camino por enlace esta
 * encendido, o sea *«viene una conexion en camino y todavia no sabemos como termina»*. Las dos fuentes
 * de «hay billetera» que este archivo tiene —el estado `address` de `:163` y el bridge de la
 * inyectada— siguen siendo dos y ninguna se toca.
 *
 * ⚠️ LAS TRES CONDICIONES HACEN FALTA, y el motivo de cada una ya esta escrito en `:4005`: `marca !==
 * null` deja invisible el cambio para todo lo que monta esta pantalla sin marca; la pertenencia a
 * `MARCAS_DE_VUELTA` impide que un `?dl=` ajeno dispare nada; y `deeplinkEnabled()` es LA MISMA env
 * del repliegue del build aplicada en un sitio mas, ⛔ no una perilla nueva.
 *
 * ⚠️ SE LEE EN EL PRIMER RENDER Y NO DESPUES: la limpieza de la barra (`:4023`) borra el `dl` con
 * `replaceState` para TODA vuelta, asi que una lectura tardia contesta `false` **siempre**, incluidas
 * las vueltas que si funcionaron. Es la misma trampa —y la misma solucion, la foto al montar— que ya
 * documenta el bloque de diagnostico en `./diagnostico-de-vuelta.tsx`.
 */
function esVueltaPorEnlaceNuestra(href: string): boolean {
  const marca = marcaDeVuelta(href);
  return marca !== null && (MARCAS_DE_VUELTA as readonly string[]).includes(marca) && deeplinkEnabled();
}
