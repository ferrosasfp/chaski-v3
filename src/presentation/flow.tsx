...
import { cn } from "./cn"; import { anotarCorteDeVuelta, anotarHito, formaDelDestino } from "./bitacora-de-vuelta"; // HU-075/diagnóstico — EN ESTA LÍNEA y no en una nueva: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=155]] citas ancladas y una línea de import de más las corre a todas. HU-075/gesto: `formaDelDestino` PEGADO ACÁ por lo mismo
...
export function RemittanceFlow({ container, pasoInicial = "bienvenida" }: { container?: Container; pasoInicial?: Step } = {}) {
  ...
  const [step, setStep] = useState<Step>(pasoInicial); useEffect(() => { anotarHito("pantalla", step); }, [step]); // HU-075/diagnóstico it2 — EL HITO DE LA PANTALLA, EN ESTA MISMA LÍNEA (Δ0: este archivo recibe [[CENSO src/presentation/flow.tsx entrantes=155]] citas ancladas por número y mide [[CENSO src/presentation/flow.tsx lineas=4453]] líneas; una línea nueva las corre a todas). 🔴 POR QUÉ HACE FALTA: el bloque de diagnóstico es HERMANO de esta pantalla en `app/page.tsx` y ⛔ no puede leer un solo `useState` de acá, así que «estoy en la bienvenida» era una descripción de una foto y no un dato — y es contra ese dato que se lee el desenlace de `onConnect`. ⛔ VA EN UN EFECTO Y NO EN EL RENDER: escribir un store de módulo durante el render es impuro y bajo StrictMode se ejecutaría dos veces. ⛔ Y NO NOTIFICA A NADIE: (`anotarHito`, `./bitacora-de-vuelta.ts:101`) es un `Map.set` sin oyentes, así que esto no re-renderiza nada, ni acá ni en el hermano — el bloque refresca solo
